...
- Unified shared chrome icon buttons across project, template, settings, and editor-panel surfaces with pressed-state motion, high-contrast border handling, disabled tinting, and consistent 48dp touch targets.
- Added tactile feedback to settings rows so dropdowns, toggles, and action tiles respond like intentional controls instead of static cards.

### Roadmap Hygiene
- Normalized blocked work into `Roadmap_Blocked.md` and reduced `ROADMAP.md` to
  actionable-only state with no open local implementation items.

### Coil 3.1.0 → 3.3.0
- Bumped Coil from 3.1.0 to 3.3.0 — adds `memoryCacheMaxSizePercentWhileInBackground` for background memory relief, custom SVG parser support, and `Uri.Builder` for URI modification. Coil 3.4.0+ requires Kotlin 2.4.0 (ClearCut uses 2.1.21) so 3.3.0 is the highest compatible version.

### Android 18 Camera Capture URI Grant Prep
- Replaced `ActivityResultContracts.CaptureVideo()` with custom `CaptureVideoWithGrant` contract that explicitly adds `FLAG_GRANT_READ_URI_PERMISSION | FLAG_GRANT_WRITE_URI_PERMISSION` to `ACTION_VIDEO_CAPTURE` intents. Android 18 will stop auto-granting these permissions.
- Audited all 6 `ACTION_SEND` intent sites — all already add `FLAG_GRANT_READ_URI_PERMISSION`. No changes needed.

### Android 17 MemoryLimiter Detection
- `ProcessExitRecorder` now detects Android 17's `MemoryLimiter:AnonSwap` description in `REASON_OTHER` exits and categorizes them as `MEMORY_LIMITER` in process-exit-history.json diagnostic ZIPs.
- JVM tests for pattern matching and JSON output.

### Room 3.0 Migration Plan Correction
- Corrected `docs/room-3-migration-plan.md` — Room 3.0 has NOT been released in any form. Prior research cycles incorrectly claimed alpha/rc releases. Document now accurately reflects Room 2.8.4 as latest stable.

### EU AI Act Article 50 Compliance
- Added `AiDisclosurePolicy` classifying all 14 AI tool IDs and 13 EffectKind values as Article 50 in-scope (substantive alteration) or exempt (assistive).
- Pre-interaction disclosure: first in-scope AI tool use per session shows a notice that the tool uses on-device AI to modify video.
- Export sidecar schema bumped to `com.clearcut.ai-use.v2` with `article50InScope` flag and IPTC `digitSourceType` URI for machine-readable AI disclosure.
- Session-level `hasShownArticle50Disclosure` flag in EditorAiState prevents repeated notices.
- Localized disclosure string (en + es).
- JVM test coverage for tool classification, EffectKind mapping, and machine-readable label requirements.

### Coil 3.1.0 Migration
- Migrated from Coil 2.7.0 (`io.coil-kt`) to Coil 3.1.0 (`io.coil-kt.coil3`).
- Package rename: `coil.compose.*` → `coil3.compose.*`, `coil.decode.*` → `coil3.video.*`, `coil.request.*` → `coil3.request.*`.
- Removed deprecated `crossfade(true)` call (now configured at ImageLoader level).
- Coil 2.x is in maintenance mode; 3.x provides better Compose integration and memory management.

### Compose BOM 2026.06.00
- Upgraded from 2026.05.01 to 2026.06.00 (Foundation 1.11.3, Material3 1.4.0, UI 1.11.3).
- Includes new SlotTable implementation (opt-in via ComposeRuntimeFlags) for faster random edits and lazy-layout recomposition.

### Lottie Reduced-Motion Accessibility
- Lottie template animations render as static final frames when Android's reduced-motion setting is enabled (ANIMATOR_DURATION_SCALE = 0).
- `LottieTemplateEngine.isReducedMotionEnabled()` added with `forceReducedMotion` parameter on `renderFrame()`.
- EU Accessibility Act (Directive 2019/882) compliance for animated content.

### PublicFeatureClaimsTest Expansion
- Extended to cover 5 previously untracked stub engines: StemSeparationEngine, VoiceCloneEngine, LipSyncEngine, EquirectangularEngine, ContentIdEngine.
- All 5 added to Roadmap_Blocked.md with concrete activation criteria.

### Toolchain Evaluations
- Room 3.0 migration plan documented (`docs/room-3-migration-plan.md`): DEFER until stable release. Strategy: `room3-sqlite-wrapper` bridge, DAO suspend conversion (~15 methods), package rename.
- Kotlin 2.4.0 upgrade evaluated (`docs/kotlin-2.4-upgrade-plan.md`): DEFER until KSP2 reaches stable. Context parameters and explicit backing fields are desirable but KSP2 beta gaps block adoption.

### WCAG 2.5.7 Drag Operation Alternatives
- Speed curve editor: tap-to-select points, numeric position/speed text fields, delete button for interior points.
- Timeline trim mode: numeric In/Out time fields (seconds or min:sec) below trim banner.
- Preview split-screen wipe: slider alternative below canvas.
- Mask editor: numeric X%/Y% fields for geometric mask control points.

### Text-Based Transcript Editing
- New TranscriptEditorPanel with scrollable word-aligned transcript, tap-to-select words for deletion, long-press-to-seek, filler word auto-selection, and ripple-cut apply.
- Accessible from clip Edit sub-menu via "Text-Based Editing" action.

### Public Claims Test Coverage
- Extended PublicFeatureClaimsTest to cover all 5 stubbed engines (TemplateMarketplace, StockAsset, CameraCapture, CaptionTranslation, TimelineImport).

### Product Trust & Release Hardening
- Reframed the local project archive/import flow from cloud-backup wording to Archive Transfer language across the app surface and release-facing copy.
- Localized the project archive export success/failure toasts so Archive Transfer feedback honors the active app language.
- Updated public README claims so archive transfer, timeline interchange, dependency, and INTERNET permission copy no longer imply unshipped cloud sync or active importer backends.
- Aligned timeline-exchange capability flags with the guarded importer state so validators do not expose OTIO import as ready before the product import path is wired.
- Replaced the startup health-ledger `GlobalScope` write with an application-owned coroutine scope so cold-start telemetry follows the app lifecycle.
- Restored the lint verification path by adding missing lint-toolchain checksums and disabling only the Compose lint detectors that crash under the current Kotlin/AGP stack.
- Fixed lint-blocking release cleanup, diagnostic version-code compatibility, and batch-export progress string formatting issues.
- Updated README lint status to reflect the restored local lint path and remaining warning debt.

## v3.74.108 - 2026-06-26

### Premium UX Polish
- Added shared press-state feedback to primary and secondary buttons so repeated calls to action feel more tactile without adding noisy motion.
- Made project cards adapt on narrow screens with smaller thumbnails, tighter spacing, and less decorative chrome so names, metadata, and actions stay readable.
- Improved settings rows with compact stacked layouts and action-specific trailing icons for destructive, refresh, drill-in, and external navigation actions.
- Tightened editor/export accessibility by adding pane titles to editor sheets, animating panel content changes, clearing duplicate switch semantics in export toggles, and aligning timeline chrome with high-contrast theme tokens.
- Cleaned up the export watermark picker so it persists read permission when available and no longer carries placeholder LocalContext code.

### Verification
- `:app:compileDebugKotlin --rerun-tasks` passes with JDK 21 and `ANDROID_HOME=C:\Users\xray\AppData\Local\Android\Sdk`.
- Runtime metadata remains synced at `versionName 3.74.108` / `versionCode 241`.

## v3.74.107 - 2026-06-16

### Premium UX Polish
- Added project-home media health and render readiness cards so first-run and returning users see concrete trust signals before opening a project.
- Refined the editor top bar hierarchy with compact semantic chips for editor mode and timeline readiness, removing the previous title offset treatment.
- Tightened compact editor chrome so search moves into More on phones, timeline tools sit below the heading, and the empty-state Add Media action renders as a labeled 48dp target.
- Aligned home/editor density more closely with the supplied premium creator-studio reference while keeping content grounded in real local state.

- Bumped runtime metadata to `versionName 3.74.107` / `versionCode 240`.

## v3.74.106 - 2026-06-16

### Premium UX Polish
- Reworked the transform overlay gesture path so one-finger move gestures are not also processed as transform pans, and pinch/rotate sessions now finalize with the normal transform-end callback.
- Made transform handles derive their geometry from the actual preview canvas instead of the stale 400x225 fallback, keeping overlay controls aligned on real phone/tablet preview sizes.
- Tightened the first-run project home rhythm so the empty studio card follows the hero instead of floating far down the screen on tall devices.

### Accessibility & Theme Consistency
- Added polite live-region and explicit progress semantics to export state cards, including determinate progress range metadata.
- Normalized export/settings card and toggle surfaces through semantic theme colors so high-contrast mode keeps consistent text, surface, and stroke treatment.
- Added JVM coverage for transform overlay geometry, normalized pan math, and single-finger vs two-finger gesture routing.

- Bumped runtime metadata to `versionName 3.74.106` / `versionCode 239`.

## v3.74.105 — 2026-06-16

### Smart Reframe
- Activated real MediaPipe BlazeFace face detection in SmartReframeEngine, replacing the stub that returned null.
- Face detection runs at 256px resolution every 500ms through the video, computing per-frame subject centers with EMA smoothing.
- The strategy resolver auto-downgrades from PAN to STATIONARY when subject movement is below threshold.
- Reframe results now generate POSITION_X/POSITION_Y keyframes on the source clip so the crop tracks the detected subject.
- Added model download support via ModelDownloadManager with SHA-256 verification for the BlazeFace model (~200KB).

### Localization
- Localized 160+ hardcoded English toast strings across EditorViewModel, V369Delegate, ClipEditingDelegate, EffectsDelegate, OverlayDelegate, ColorGradingDelegate, and ExportDelegate.
- Added Spanish translations for all extracted strings.
- Zero hardcoded English toast strings remain in the editor layer.

### Toolchain & CI
- Completed Room 2.8.4, Kotlin 2.1.21, KSP 2.1.21 toolchain migration (verified clean compile + KSP schema generation).
- Added SHA-256 Gradle dependency verification for 5132 Maven artifacts.
- Added Android Lint CI lane with baseline and HTML/SARIF report upload.
- Added Compose compiler metrics output for recomposition stability analysis.
- Added Gradle dependency graph submission for Dependabot advisory scanning.

### Testing
- Added FFmpegEngine JVM tests (13 tests: atempo chaining, path escaping, time formatting).
- Added SmartReframeEngine JVM tests (13 tests: EMA smoothing, strategy resolution).

### Media Durability
- Added lazy background SHA-256 hashing for managed media assets via MediaHashWorker (WorkManager).
- Corrected stale SmartRenderEngine "not used for export" claim — the mixed-render export path was already fully wired.

- Bumped runtime metadata to `versionName 3.74.105` / `versionCode 238`.

## v3.74.104 — 2026-06-16

### Stock Assets
- Added provider capability metadata for Pexels, Pixabay, Freesound, and Free Music Archive so stock search remains disabled until provider prerequisites are configured.
- Added per-provider terms for source branding, attribution, cache TTL, hotlinking, commercial-use, and derivative-use policy.
- Expanded stock asset metadata with source URL, license URL, fetched timestamp, and reuse constraints, plus export-use decisions that block noncommercial and no-derivatives assets when needed.
- Bumped runtime metadata to `versionName 3.74.104` / `versionCode 237`.

## v3.74.103 — 2026-06-16

### Camera Capture
- Reframed Add Media camera capture as an external camera-app handoff: the UI now says ClearCut opens the camera app, receives only the returned clip, and does not request camera permission for this path.
- Added `CameraCaptureCapability` metadata that keeps the shipped external handoff separate from the future CameraX in-app recorder capability.
- Added focused JVM tests for camera-handoff copy, the absent `CAMERA` permission, optional camera feature policy, and the CameraX recorder gate.
- Bumped runtime metadata to `versionName 3.74.103` / `versionCode 236`.

## v3.74.102 — 2026-06-16

### Platform Handoff
- Reframed the former Direct Publish creator card as Platform Handoff so the UI says ClearCut opens another app with suggested post text instead of implying API-backed publishing.
- Added explicit share-only platform capability metadata to `DirectPublishEngine`; API upload now remains visibly unavailable until OAuth consent, credential storage, platform approval, and resumable upload adapters exist.
- Added Android Sharesheet fallback behavior when a target app is not installed and included manual AI-disclosure reminders in the share text for platforms that expose their own AI controls.
- Bumped runtime metadata to `versionName 3.74.102` / `versionCode 235`.

## v3.74.101 — 2026-06-16

### Release Trust
- Added GitHub artifact attestations for APKs, checksum sidecars, and signing-certificate fingerprint sidecars.
- Release CI now verifies downloaded APK checksums, signing fingerprints, and artifact attestations before uploading tag assets.
- Documented direct APK verification with `gh attestation verify`, checksum validation, and APK signing-certificate fingerprint checks.
- Bumped runtime metadata to `versionName 3.74.101` / `versionCode 234`.

## v3.74.100 — 2026-06-15

### Correctness
- Fixed potential crash in WhisperEngine audio decode when a corrupt media file reports zero channels — division-by-zero in mono downmix now guarded with an early return.
- Replaced force-unwrap (`!!`) on ModelDownloadManager SHA-256 field with `requireNotNull` to produce a clear diagnostic instead of bare NPE if the invariant is violated.
- Added zero-duration guard to `Clip.relinkedTo()` — prevents `IllegalArgumentException` from `coerceIn(0, -1)` if a degenerate 0ms media file is used as a relink target.

### I18n
- Replaced hardcoded English "Could not read media file" toast in clip add path with localized string resource (`editor_media_read_failed_toast`).

### Diagnostics
- Added Log.w to the silent catch in ExportDelegate audio-conformance probe so MediaExtractor failures during export preflight are visible in logcat.

## v3.74.99 — 2026-06-15

### Accessibility / UX
- Localized the remaining AI tool status/result toasts, including scene detection, captions, crop/color, stabilization, denoise, background tools, tracking, style transfer, reframe, and upscale feedback.
- Replaced unfinished AI tool feedback such as pending object-removal painting and OpenCV stabilization implementation copy with releasable fallback messages.

### Release
- Bumped runtime metadata to `versionName 3.74.99` / `versionCode 232`.

## v3.74.98 — 2026-06-15

### Accessibility / UX
- Replaced high-visibility AI and audio mixer error toasts with localized, user-safe feedback that avoids raw exception text.
- Replaced raw exception text in media import, LUT import, video export/save, backup import/export, project archive, OTIO/FCPXML, multi-cam sync, smart reframe, Cut Assistant, auto-duck, and template import/export failure messages with safer localized copy.
- Localized AI model download/template dispatch feedback and audio beat/normalization/mastering feedback.
- Corrected Spanish short-form template and auto-edit duration labels that read as years instead of seconds.

### Release
- Bumped runtime metadata to `versionName 3.74.98` / `versionCode 231`.

## v3.74.97 — 2026-06-15

### Accessibility / UX
- Added root IME padding so project/search/editor forms avoid keyboard overlap while staying inside safe drawing insets.
- Project search now advertises a Search keyboard action, and project rename supports Done-to-submit only when the trimmed name is valid.

### Localization
- Settings update-check, diagnostic ZIP, and model-storage feedback now resolve through English and Spanish resources instead of hardcoded ViewModel strings.

## v3.74.96 — 2026-06-15

### Accessibility / UX
- Preview overlay controls, the preview play/pause button, and top-level timeline toolbar controls now reserve 48dp interaction targets.
- Timeline toolbar controls scroll horizontally on narrow screens instead of squeezing the timeline title area.
- Mini Player play/pause content descriptions now resolve through localized resources, and time labels clamp to one line with ellipsis.

## v3.74.95 — 2026-06-15

### Reliability
- Added committed-schema Room migration instrumentation coverage for the project database from schema v4 through v8. Schemas v1-v3 are not present under `app/schemas`, so full 1→8 coverage remains tracked in the roadmap.

### Accessibility / i18n
- Storyboard edit/delete controls now reserve 48dp touch targets, and storyboard labels, action content descriptions, and project trash restore/delete/empty-trash feedback now resolve through English and Spanish resources.

## v3.74.94 — 2026-06-14

### Feature
- Enum display names (KaraokeStyle, DirectPublish Target, ColorBlind Mode)
  now resolve through string resources at the UI boundary instead of rendering
  hardcoded English `.displayName` properties. Added matching Spanish
  translations. SortMode and FilterMode were already localized.
  AspectRatio labels ("16:9" etc.) are universal notation and unchanged.
- Independent head and tail transitions per clip. Clips now carry separate
  `headTransition` (transition-in at clip start) and `tailTransition`
  (transition-out at clip end) fields. When a clip has an explicit tail
  transition, it takes priority over the next clip's head transition for the
  out-effect. Autosave reads the legacy `"transition"` key as `headTransition`
  for backward compatibility with existing projects. All transition consumers
  (export, preview, EDL, timeline rendering, diagnostics, template validation,
  exchange validation) updated. LumaFusion v2.5 parity.
- One-tap "Normalize All Clips" action in Audio Normalization panel. Measures
  loudness across all audio and video track clips, computes per-clip gain to
  reach the selected LUFS target with true-peak limiting, and applies all
  volume adjustments as a single undoable edit. The existing single-clip
  normalize button is disabled when no clip is selected.

### Security / CI
- GitHub Actions workflow permissions scoped to least privilege: top-level
  default is now `contents: read`; only the new `release` job (tag-triggered,
  depends on `build`) elevates to `contents: write`. Build, test, verification,
  and instrumentation-smoke jobs no longer hold a write token. Pull-request runs
  expose no write token at all.
- All GitHub Actions pinned to immutable commit SHAs instead of mutable version
  tags. Dependabot's `github-actions` ecosystem still proposes updates, but a
  compromised tag can no longer silently change what CI executes.
- Release upload isolated into a dedicated `release` job that only runs on
  `v*` tags, downloads build artifacts from the `build` job, and creates the
  GitHub Release — keeping the write token out of the build/test surface.

## v3.74.93 — 2026-06-14

### Reliability
- Fixed a resource leak in `LottieTemplateEngine.loadDotLottie`: the dotLottie
  ZIP stream is now closed on every exit path, including coroutine cancellation
  (where neither Lottie listener fires) and synchronous load failures. Both
  Lottie loaders now rethrow `CancellationException` instead of swallowing it in
  the generic catch.

### Data safety
- `clearBeatMarkers` now records an undo snapshot (and no-ops when already
  empty) so clearing detected beats is reversible like every other timeline
  edit, instead of wiping silently.

### Accessibility
- The caption-translation "Regenerate" control now reserves a 48dp minimum
  touch target.

### Accessibility / i18n
- Completed the Spanish locale: added the 30 missing `es` translations
  (Project Inspector, before/after preview comparison, storyboard and
  project-inspector tool labels, audio-conformance and insufficient-space
  notices, locked-clip description) that recent features shipped English-only.
  Restores the `LocaleResourceCoverageTest` gate to green so Spanish no longer
  silently falls back to English on those surfaces.
- Split content-description resources that were being reused as visible text
  into purpose-named strings so translators can word the spoken and visible
  forms independently. The Auto Edit card heading now uses
  `auto_edit_generate_title` (separate from the icon's `cd_auto_edit_generate`
  description), and the V369 readiness "Selected" state uses
  `v369_readiness_clip_selected` (separate from `cd_check_mark`). Added matching
  English and Spanish strings.

### Feature
- Passive, opt-in update check for sideload / GitHub-release installs. A new
  Settings → Updates section lets users turn on a single, TLS-only version
  lookup against the public GitHub releases API and, when a newer release
  exists, link straight to the release page. ClearCut never downloads or
  installs an APK, so it stays clear of the F-Droid self-updater anti-feature.
- Off by default: with the toggle off (or a build that sets
  `UPDATE_CHECK_AVAILABLE=false`, e.g. an F-Droid fork) no network request is
  ever made. The privacy dashboard discloses the endpoint as an opt-in
  cloud/telemetry row.

## v3.74.92 — 2026-06-13

### UX
- ToolPanel tab row spacing normalized from 3dp to 4dp for consistent rhythm
- ToolPanel LazyRow content spacing increased from 2dp to 4dp with matching
  start padding for better touch target separation
- ToolPanel sub-menu drag handle enlarged from 4dp to 5dp height with
  design-token radius for improved grab affordance on phone layouts
- Timeline lock icon now has accessible content description for screen readers

## v3.74.91 — 2026-06-13

### Feature
- Project Inspector panel: shows media counts (clips, video/audio/overlay tracks,
  text overlays, effects, keyframes), project duration, missing media warnings,
  export settings (resolution, codec, frame rate), and storage/recovery info
  (auto-save size, last saved, backup files, schema version). Accessible from
  Project Tools sub-menu. Uses Mocha.Blue accent with Mocha.Red issue warnings.

## v3.74.90 — 2026-06-13

### Research
- ffmpeg-kt evaluation spike: DEFER. zt64/ffmpeg-kt is a raw libav* KMP
  binding (pre-alpha, no stable release), not a drop-in FFmpegKit replacement.
  Zero API overlap with ClearCut's command-string call sites, no SAF support,
  no 16KB compliance docs. Current moizhassankh fork adequate; JamaisMagic
  fork (io.github.jamaismagic.ffmpeg, 6.1.7, LGPL option) identified as
  drop-in upgrade path if needed. Full evaluation in FFmpegEngine.kt KDoc.

## v3.74.89 — 2026-06-13

### Feature
- Split-screen before/after comparison for effects and color grading.
  Toggle button (Compare icon, Teal accent) in preview overlay appears
  when the selected clip has active effects or a color grade. Draggable
  vertical wipe divider shows the original uneffected source frame on
  the left and the live effected preview on the right. Original frame
  captured via extractThumbnail at source-time resolution, cached per
  playhead position. "Original" / "Edited" labels in bottom-left corner.

## v3.74.88 — 2026-06-13

### Feature
- dotLottie v2 / state-machine compatibility spike completed in LottieTemplateEngine.
  `loadDotLottie(uri)` method added for loading `.lottie` ZIP containers via
  `LottieCompositionFactory.fromZipStream()` (supported in lottie-android 6.7.x).
  Zero APK impact — lottie-compose 6.7.1 is already a dependency. Findings:
  dotLottie container loading ADOPTED (10-15x smaller files than raw JSON),
  text/color replacement WORKS via TextDelegate + LottieValueCallback,
  state machines DEFERRED (no public API in 6.7.x, blocked on upstream 7.x),
  Rive REMAINS INACTIVE until Lottie state machines are confirmed unavailable.

## v3.74.87 — 2026-06-13

### Feature
- Storyboard shot-plan workspace for auto-edit: plan shots with text descriptions,
  status chips (Planned/Filmed/Edited), and target durations. Cards persist across
  sessions via AutoSaveState serialization/deserialization. Storyboard cards
  automatically seed AutoEditPanel's script field when no manual script is provided.
- `StoryboardPanel` — new PremiumEditorPanel composable with inline card add/edit,
  status cycling, remove, and summary stats.
- `StoryboardCard` + `StoryboardCardStatus` data model in EditorModels.kt.
- ViewModel methods: addStoryboardCard, updateStoryboardCard, removeStoryboardCard,
  reorderStoryboardCards, showStoryboard, hideStoryboard — all with undo support.
- Wired into ToolPanel projectToolsSubMenu ("Storyboard") and EditorScreen action
  dispatch. Recovery, snapshot restore, and backup import paths all hydrate
  storyboard cards.

## v3.74.86 — 2026-06-13

### Feature
- `.ncstyle` caption and text style packs: importing `.ncstyle` files validates
  JSON schema version, duplicate IDs, style count limits, and required fields.
  Valid packs install to a local registry and appear alongside built-in styles in
  the caption/text style gallery. Invalid packs show a structured rejection report
  and mutate no project state. Installed packs can be listed and removed.
- `StylePackManager` — Hilt singleton that validates, installs, lists, and removes
  `.ncstyle` JSON packs. Parses all 16 `CaptionStyleTemplate` fields with
  clamped numeric ranges and graceful enum fallbacks.
- `IncomingDocumentImportRouter` — STYLE_PACK case upgraded from BLOCKED stub to
  full preview + import flow. Preview validates without installing; import writes
  pack to `filesDir/style_packs/` and reports style count and author.
- `CaptionStyleGallery` now merges installed pack styles alongside built-in
  templates, auto-categorized into accessibility, karaoke, and editorial sections.
- `IncomingDocumentKind.STYLE_PACK` target action updated from "Review pending"
  to "Install to Caption Gallery".

## v3.74.85 — 2026-06-13

### Engine
- Added `NativeProcessingPolicy` — shared input-validation gate for all FFmpeg
  and ONNX entry points. Enforces max input file size (4 GB video, 512 MB audio,
  50 MB image, 10 MB subtitle), supported MIME type and extension allow-lists,
  and per-operation timeout constants. Oversized, unsupported, or corrupt inputs
  are rejected before native processing begins.
- Wired policy validation into all 9 public FFmpegEngine entry points
  (`extractAudioToWav` ×2, `reverseClipToFile`, `extractAudioToPcm16le`,
  `encodePcm16leToM4a`, `burnSubtitles`, `normalizeLoudness`, `streamCopyTrim`,
  `concat`, `changeSpeed`). Each validates size and format before dispatching to
  FFmpegKit.
- Wired policy validation into InpaintingEngine: `inpaintFrame()` rejects
  bitmaps exceeding the image byte limit; `inpaintVideo()` validates the source
  URI against video size and format policy. ModelDownloadManager (SHA-256 +
  size gate) was already hardened and remains unchanged.
- All policy violations produce both a sanitized user-facing message (no file
  paths or byte counts) and a detailed diagnostic log line (Log.w with operation
  name, actual vs. limit bytes, detected MIME/extension).

## v3.74.84 — 2026-06-13

### Audio Engine
- Added audio conformance preflight to the export pipeline: detects mixed
  sample rates, mixed channel counts, and uncommon sample rates across all
  clips before export begins. Reports issues as warnings in the export
  preflight result.
- Added pure-Kotlin linear-interpolation resampler (`resampleLinear`) and
  channel converter (`convertChannels`) to AudioEngine as a fallback when the
  Oboe sinc resampler is unavailable.
- Wired resampling into `mixAudioTracks()`: decoded PCM is now normalized to
  the output sample rate and channel count before mixing, fixing silent
  desync when 44.1 kHz music is mixed with 48 kHz video audio.
- Added `probeAudioFormat()` for extracting sample rate, channel count, MIME,
  and duration from any media URI via MediaExtractor.
- Added `AudioConformanceReport`, `AudioConformanceIssue`, `AudioFormatInfo`
  data classes for structured conformance reporting.
- Extended `AudioTrackData` with `sourceSampleRate` and `sourceChannels`
  fields so callers can pass probed metadata through the mix path.

## v3.74.83 — 2026-06-13

### Media Import
- Added background ingest pipeline via WorkManager: large media imports now run
  in the background with progress reporting, cancellation support, and automatic
  clip insertion on completion.
- Added free-space preflight check before all import paths (single, batch, drop):
  verifies available storage exceeds file size plus a safety margin (50 MB or 10%,
  whichever is larger) before copying.
- Added chunked copy with cancellation support: 64 KB buffer with per-chunk
  cancellation checks and 1%-granularity progress callbacks.
- Added `IngestResult` sealed class for structured import outcomes (Success,
  InsufficientSpace, Cancelled, Failed).
- Added `PendingIngest` state in `EditorMediaState` for UI progress tracking.

## v3.74.82 — 2026-06-13

### Performance
- Added persistent disk-backed waveform cache: extracted waveform peak data
  is now persisted to `filesDir/waveform-cache/` so project reopens load
  waveforms from disk instead of re-decoding PCM audio. Bounded at 50 MB
  with LRU eviction, excluded from cloud backup.

## v3.74.81 — 2026-06-13

### Export UX
- Added an in-editor export playback preview: after a successful export, an
  ExoPlayer-backed video surface appears in the export completion sheet so
  users can verify the output before sharing or saving. The preview player
  releases when the sheet dismisses.
- Added an export settings suggestion chip that appears when the selected
  resolution or frame rate differs from the project's native settings,
  with an upscale warning when export resolution exceeds source.

### Editing UX
- Added composition guides overlay to the preview panel: toggleable
  rule-of-thirds grid, title-safe (80%) and action-safe (90%) zone outlines,
  and a center crosshair. Toggle button sits alongside the scopes button.
- Added haptic feedback during timeline ruler scrub: light pulses fire when
  the playhead crosses a clip boundary, timeline marker, or beat marker.

### Export fidelity
- Fixed animated GIF overlay timing in export: replaced wall-clock-driven
  AnimatedImageDrawable with Movie.setTime() for presentation-time-driven
  frame selection. GIF overlay timing is now deterministic regardless of
  encode throughput.

### Build fixes
- Fixed three pre-existing compilation failures: missing Material Icons
  imports in CommandPalette, editorOnAction lambda scope mismatch in
  EditorScreen, and missing Warning icon import in ExportSheet.
- Synced 6 missing string keys to the Spanish locale.

## v3.74.78 — 2026-06-13

### Export reliability
- Added export failure incident bundles: failed, timed-out, and blocked exports
  now record a bounded, redacted JSON incident with app version, device model,
  encoder path (hardware vs software), codec, resolution, progress samples,
  elapsed time, media health summary, and error class/message. Incidents are
  stored locally under `filesDir/diagnostics/export-incidents/`, pruned at 10,
  included in user-triggered diagnostic ZIPs, and never uploaded.
- Bumped runtime metadata to `versionName 3.74.78` / `versionCode 215`.

## v3.74.77 — 2026-06-12

### Premium UX polish
- Tightened shared control chrome with smaller-radius buttons, compact status
  labels, icon buttons, dialog icons, and panel cards across home, editor, and
  settings.
- Refined the editor bottom tool bar and sub-menu grid with rectangular icon
  tiles, more legible compact labels, and clearer selected/disabled states.
- Improved project home and settings hierarchy with stronger primary-action
  weight, sharper empty states, and explicit navigation affordances.
- Bumped runtime metadata to `versionName 3.74.77` / `versionCode 214`.

## v3.74.76 — 2026-06-11

### Media reliability center groundwork
- Autosave restore now prefers `mediaAssets[assetId].managedUri` over stale
  serialized clip source URIs, so saved projects route preview/export through
  the app-owned managed file when a stable asset ID exists.
- Autosave, recovery health checks, and export media preflight now reuse a
  media-reference-keyed asset manifest cache, avoiding repeated sidecar,
  metadata, and fingerprint scans during trim, timeline, and slider-heavy
  editing when the media set has not changed.
- Export attempts now write a bounded local history with completed, failed,
  blocked, and cancelled outcomes, including output metadata, elapsed time,
  media-health counts, and diagnostic summaries that appear in the Export
  sheet for faster retries and support.
- Managed-media cleanup scans autosave asset manifests for `managedUri` values
  as well as clip `sourceUri` values, preventing live app-owned imports from
  being reclaimed when older clip JSON still contains stale provider URIs.
- Added an always-visible edit-confidence rail under the editor top bar that
  summarizes undo/redo depth, restore-point count, and autosave readiness with
  direct entry points into History and Version History.
- Preview-critical edits now use a shared preview-first phone layout policy:
  trim, slide/slip drags, crop, transform/motion, speed, masks, color, captions,
  and effect adjustments reserve a larger preview, cap timeline chrome, and
  keep the bottom tool rail compact on single-pane phones.
- Added a project-level media health engine that classifies managed media,
  missing or empty local files, external/provider-backed sources, duplicate
  asset IDs, unknown asset references, and stale manifest URIs into blocking
  and warning findings.
- Saved-project restore now stores the media health report in the editor media
  domain state, giving the UI a canonical project-health signal alongside
  clip-level relink probes.
- The Media Manager now shows a project-health metric for restore-time blockers
  and warnings, with English and Spanish resources covered by locale tests.
- Current editor scans now refresh project media health, missing preview proxies
  surface as non-blocking warnings, and export attempts are blocked before
  rendering when media health or relink probes find blocking source issues.
- Recovery opens now surface manifest-health blockers and warnings through the
  same Media Manager path used for missing or unverified source files.
- The autosave recovery dialog now summarizes restored media readiness,
  repair blockers, proxy fallback, or warnings before the user continues
  editing.

### Media restore hardening
- Saved-project archive imports now rewrite the `mediaAssets` manifest to the
  extracted media files, matching rewritten clip and overlay URIs so restored
  projects keep playback metadata aligned with the actual local files.
- Imported media assets now fall back from stale provider `originalUri` values
  to the extracted managed file instead of reporting false missing-media
  diagnostics after archive restore.
- Managed media sidecar reads now reject empty, malformed, or oversized asset
  JSON before it can poison save-state reconstruction.
- Managed media cleanup now reclaims orphaned `.asset.json` sidecars after the
  corresponding media file is gone, while preserving sidecars for live media.
- Added regression coverage for malformed sidecars, empty managed media,
  empty proxies, nested archive media rewrites, and unresolved archive media.
- Added a Room-backed `project_media_assets` manifest table with v7-to-v8
  migration coverage, plus save/import/duplicate synchronization from the
  autosave media manifest.

## v3.74.72 — 2026-06-11

### Saved-project media playback
- OpenDocument media picks now copy into ClearCut's managed media directory
  before they are added to the timeline, matching Photo Picker and drag/drop
  behavior so saved projects do not depend on a provider grant that can vanish
  after edits or process restarts.
- Preview playback now validates a restored proxy URI before using it. If the
  proxy cache file was cleaned up, truncated, or otherwise unreadable, the
  player falls back to the original source URI instead of preparing an
  unplayable saved-project timeline.
- Bumped runtime metadata to `versionName 3.74.72` / `versionCode 209`.

## v3.74.71 — 2026-06-11

### Editor phone layout
- Reserved a usable preview height during compact phone clip editing so edge
  trim/slide gestures no longer collapse the video into a tiny strip while a
  tall timeline and bottom tool rail consume the screen.
- Fixed the bottom tool area measuring at its expanded 236dp height when no
  submenu was open, which left a large empty block under the buttons.
- While dragging clip edges, the preview now seeks to the active trim boundary:
  left edge shows the new in-frame and right edge shows the last retained
  out-frame so users can tell where to stop. Trim drags keep normal preview
  decode enabled instead of ExoPlayer scrubbing mode so the frame is visible
  during the drag.
- Lowered compact clip-mode timeline caps and tied the timeline budget to the
  reserved preview height, keeping bottom controls from floating above a large
  empty editor area.
- Bumped runtime metadata to `versionName 3.74.71` / `versionCode 208`.

## v3.74.70 — 2026-06-11

### Spanish locale
- Added a complete Spanish (`es`) string resource set mirroring the base UI
  strings and registered it in Android's per-app language configuration.
- Added JVM coverage to prevent the Spanish locale from missing future string
  or plural resources and falling back to English silently.
- Updated README language availability and bumped runtime metadata to
  `versionName 3.74.70` / `versionCode 207`.

## v3.74.69 — 2026-06-11

### Deep audit pass (engineering + UX)

#### Correctness
- **Export cancellation is now honored across all phases.** Cancelling during
  the FFmpeg reversed-clip pre-render previously kept rendering every queued
  clip, then launched a detached full encode whose state-guarded listener never
  fired — leaking reversed-clip temp files in cache and burning CPU/battery to
  the end of the encode. The pre-render loop now stops between clips, the
  transformer refuses to start after cancellation, and a cancel during encoding
  signals the caller so per-export scratch files are always deleted
  (`ensureMixedExportActive` generalized to `ensureExportActive` for all paths).
- **Mask morphs no longer drop points.** Interpolating between mask keyframes
  with different point counts truncated to the smaller outline (corrupting the
  shape mid-morph in preview and export, and erasing the mask entirely against
  an empty keyframe). Outlines are now resampled to the larger count so every
  point participates; empty keyframes snap instead of erasing.
- **Keyframe precision fields work in comma-decimal locales.** Time/value
  fields formatted with the device locale ("1,5") but parsed with
  `toDoubleOrNull()`, which rejects commas — every keystroke silently failed in
  most European locales. Fields now format dot-stable, accept both separators,
  reject NaN/Infinity, and request the decimal soft keyboard.
- **Animated GIF/WebP export overlays stop their drawable on release** (the
  REPEAT_INFINITE animation previously kept scheduling main-looper frame
  callbacks after export finished) and clean up on decode-failure paths.

#### UX / safety
- **Permanent deletions now confirm.** "Empty trash" purged every trashed
  project in one tap (sitting directly beside the expand/collapse toggle) and
  per-project "Delete forever" was equally instant — both now use the standard
  confirmation dialog; the trash is the undo path, so destroying it can't ride
  on a mis-tap.
- **153 hardcoded user-facing strings externalized** to string resources across
  AudioNormPanel, BeatSyncPanel, SmartReframePanel, NoiseReductionPanel,
  AutoEditPanel, V369FeaturesPanel, the project-trash UI, and the keyframe
  editor — including proper plurals for count-based copy (trash retention,
  chapter candidates, flash-risk segments).

#### Performance / hardening
- Auto-save skips the temp-write + double-rename + backup churn when the
  serialized project is unchanged (previously rewrote the full JSON every 30s
  for idle projects).
- Timeline drag handlers no longer allocate a flattened clip list per pointer
  event (~60–120Hz during trims/slides).
- Release builds strip `Log.v`/`Log.d` via R8 (`-assumenosideeffects`);
  warning/error diagnostics are kept deliberately.
- Apache license URLs switched to HTTPS.
- Bumped runtime metadata to `versionName 3.74.69` / `versionCode 206`.

#### Tests
- New `MaskPointInterpolationTest` (mismatched/empty/single-point morphs,
  bezier handles) and `DecimalInputsTest` (comma/dot parsing, garbage
  rejection, format round-trip).

## v3.74.68 — 2026-06-11

### Trim layout: no more dead panel space
- Fixed the bottom tool rail appearing to take over half the screen during
  edge-drag trims: the timeline card was force-stretched to fill all leftover
  height while its tracks wrap top-aligned, rendering the slack as a giant
  empty panel directly above the same-colored rail.
- The preview is now the only flexible element on the editor screen — it
  absorbs leftover height while the timeline wraps its track stack between
  min/max bounds and the tool rail hugs the bottom edge.
- Edge-drag trim gestures no longer resize anything mid-drag: pane sizing and
  the tool-rail compact lock now react only to the deliberate TRIM tool mode,
  not to the live gesture (which previously collapsed an open sub-menu and
  shrank the preview the instant a clip edge was grabbed).
- Track stacks taller than the timeline's height cap now scroll vertically
  instead of clipping behind the tool rail.
- Bumped runtime metadata to `versionName 3.74.68` / `versionCode 205`.

## v3.74.67 — 2026-06-11

### Editor layout balance
- Let the phone editor timeline use remaining vertical space so linked video
  and audio tracks render as distinct, readable layers instead of compressing
  the audio waveform into a thin strip.
- Increased selected-clip preview allocation and reduced preview chrome padding
  so the canvas has more usable room.
- Tightened the bottom tool tab bar to remove excess empty toolbar space while
  preserving accessible tap targets.
- Bumped runtime metadata to `versionName 3.74.67` / `versionCode 204`.

## v3.74.66 — 2026-06-11

### Premium creator workflow polish
- Reworked the Creator Intelligence hub with premium sheet chrome, readiness
  status cards, consistent feature-card anatomy, calmer microcopy, and tokenized
  typography/buttons.
- Upgraded project trash from a bare row into a recovery panel with retention
  copy, clearer empty-trash affordance, and distinct restore/permanent-delete
  icon states.
- Renamed the phone overflow/sidebar hub entry to Creator Intelligence so the
  navigation label matches the destination.
- Bumped runtime metadata to `versionName 3.74.66` / `versionCode 203`.

## v3.74.65 — 2026-06-11

### Transition easing controls
- Added per-transition easing selection (Linear, Ease In, Ease Out, Ease In/Out)
  with FilterChip selector in the transition picker panel.
- Easing curves apply to both transition-in and transition-out effects via
  Kotlin-level time remapping in ShaderProgram, avoiding GLSL shader changes.
- Easing persisted in project autosave and survives round-trip.

### Keyframe sub-frame precision editing
- Added numeric time (seconds) and value input fields when a keyframe is
  selected, enabling exact positioning to millisecond precision.
- Bumped runtime metadata to `versionName 3.74.65` / `versionCode 202`.

## v3.74.64 — 2026-06-11

### Android 17 audio hardening gate
- Added a static Android audio API policy validator with fixture self-tests and
  CI wiring.
- Locked `ExportService` to media-processing-only behavior so it cannot start
  TTS, microphone, playback, or audio-focus APIs from a background foreground
  service path.
- Documented the Android 17 `cmd audio set-enable-hardening throw` smoke test
  for TTS preview, voiceover recording, and export.
- Bumped runtime metadata to `versionName 3.74.64` / `versionCode 201`.

## v3.74.63 — 2026-06-11

### Distribution readiness gate
- Added a repeatable distribution-readiness validator with fixture self-tests
  and CI wiring.
- Documented GitHub Releases, Google Play metadata, F-Droid metadata, and
  Android developer-verification status without claiming account registration
  is complete.
- Added Fastlane distribution-status copy so store metadata stays honest about
  direct APK, Play, and F-Droid readiness.
- Bumped runtime metadata to `versionName 3.74.63` / `versionCode 200`.

## v3.74.62 — 2026-06-11

### APK size budget gate
- Added a committed APK size baseline and checker with a 2 MB per-output growth
  allowance for debug, release, and androidTest APKs.
- Wired the size-budget self-test and check into CI before Play listing and
  release publication steps.
- Documented the deliberate baseline refresh workflow in README.
- Bumped runtime metadata to `versionName 3.74.62` / `versionCode 199`.

## v3.74.61 — 2026-06-11

### Public feature claim guard
- Added a JVM regression test that keeps the README slip/slide editing claim
  backed by `EditorScreen`, `Timeline`, and `EditorViewModel` wiring.
- Bumped runtime metadata to `versionName 3.74.61` / `versionCode 198`.

## v3.74.60 — 2026-06-11

### Release checksum provenance
- Added a repeatable SHA-256 checksum writer/checker for debug, release, and
  androidTest APK outputs.
- Updated CI to run the checksum self-test, write sidecar files before artifact
  upload, verify them, and attach the release checksum sidecar to tagged
  GitHub releases.
- Documented the local checksum reproduction commands in README.
- Bumped runtime metadata to `versionName 3.74.60` / `versionCode 197`.

## v3.74.59 — 2026-06-11

### Public release claim gate
- Added a deterministic public-claim validator with fixture self-tests and CI
  wiring alongside Play listing validation.
- Corrected README and Play metadata so current AI tools, planned/gated tools,
  offline editing, and optional network paths stay distinct.
- Updated Play screenshot source copy from structured share-target disclosure
  readiness to AI disclosure sidecar/manual platform reminder wording.
- Bumped runtime metadata to `versionName 3.74.59` / `versionCode 196`.

## v3.74.58 — 2026-06-11

### Network security hardening
- Added an app-wide `networkSecurityConfig` and explicit
  `usesCleartextTraffic=false` manifest policy so platform HTTP stacks reject
  cleartext traffic by default.
- Added JVM coverage that checks the manifest and network security XML stay
  wired together.
- Bumped runtime metadata to `versionName 3.74.58` / `versionCode 195`.

## v3.74.57 — 2026-06-11

### Open-source license notices
- Added a Settings entry for third-party notices that lists bundled dependency
  licenses and source links.
- Surfaced FFmpegKit / FFmpeg GPLv3 obligation copy and the packaged source
  offer text in-app.
- Added JVM coverage that keeps the notices registry non-empty and requires
  FFmpegKit, DeepFilterNet, MediaPipe, Lottie, and OkHttp notices.
- Updated README distribution guidance and Fastlane release notes for the
  license surface.
- Bumped runtime metadata to `versionName 3.74.57` / `versionCode 194`.

## v3.74.56 — 2026-06-11

### Audio focus and noisy-device handling
- Routed preview playback through shared Media3 audio attributes with built-in
  audio-focus and becoming-noisy handling so timeline audio no longer ignores
  competing apps or headphone disconnects.
- Added transient ducking focus for TTS preview and transient-exclusive focus
  for voiceover recording, with focus abandoned on completion, error, stop, and
  editor teardown.
- Paused timeline preview before TTS preview or voiceover recording starts, and
  stopped active TTS preview before microphone capture.
- Added focused JVM coverage for ClearCut's audio-focus policy and refreshed the
  tracked-files audit to enforce the current local-only markdown policy.
- Updated README planning references and added a physical-device audio-focus QA
  checklist.
- Bumped runtime metadata to `versionName 3.74.56` / `versionCode 193`.
- Verification: focused `AudioFocusPolicyTest`, full `:app:testDebugUnitTest`,
...
- Removed the broad managed-media FileProvider exposure and kept imported media
  private to ClearCut-managed storage.
...
- Migrated ClearCut's C2PA training/data-mining assertion from the old
...
- Rejected GIF overlay insertion with explicit user-facing copy until ClearCut
...
- Replaced the Settings DataStore delegate with a `PreferenceDataStoreFactory`
  ...
  `clearcut_settings` files recover to defaults and remain writable.
...
- Routed implemented validation through existing loaders for templates,
  ...
  ClearCut archives, and timeline import status; timeline/style-pack imports
...
- Added a persisted `AppearanceMode` setting with System (dark canvas), Dark,
  and High Contrast Dark options, wired through the root `ClearCutTheme`.
- Added high-contrast Material and shared chrome tokens through
  `LocalClearCutColors`, boosting semantic text, outlines, cards, buttons,
...
- Added `ClearCutAppearancePolicyTest` coverage for System-to-Dark resolution,
...
- Added the Compose accessibility test artifact and enabled root accessibility
  checks in `ClearCutSmokeTest` for gallery, editor empty state, media picker,
...
- Added `ClearCutApp.onTrimMemory(...)` dispatch through a tested
...
- Added a dedicated `instrumentation-smoke` GitHub Actions job that boots a
  hosted Android emulator and runs the existing `ClearCutSmokeTest` through
  `connectedDebugAndroidTest`.
- Filtered the instrumentation run to
  `com.novacut.editor.ClearCutSmokeTest` so the gate covers project gallery,
...
- Installed the recorder from `ClearCutApp.onCreate()` as the global
...
- Kept export available when notifications are declined, with fallback copy that
  tells the user to keep ClearCut open for in-app progress and cancel controls.
...
- Added shared `ClearCutTestTags` constants and stable Compose test tags for the
...
- Added Compose instrumentation dependencies and
  `ClearCutSmokeTest.projectEditorExportAndSettingsSurfacesOpen()`, which opens a
...
do not push from this VM (see [[clearcut-local-remote-divergence]]).
...
- **R6.10d — Media3 ProgressSlider evaluation.** Added
  ...
  ClearCut's `onSeek(Long)` callback.
...
- **R6.10b — Media3 Compose Player evaluation.** Added
  ...
  why ClearCut should keep its custom `PreviewPanel` for Media3 1.10.1. The
  Material3 `Player` / `ProgressSlider` path is player-timeline based and
  performs native seeks internally; ClearCut's editor preview still needs
...
- **R7.5 / R6.10a — Media3 Lottie renderer adoption.** The app now
  ...
  keeps ClearCut's existing title semantics: overlay-relative animation time,
...
- **R8.7 — Per-app language preferences.** AndroidManifest now declares
  ...
  `res/values-<bcp47>/strings.xml`, then append one `<locale>` entry. ClearCut
...
- **R8.6 / R8.13 — Photo Picker + Auto Backup audits closed.** Photo
  ...
  reframed and README's permission table corrected (ClearCut requests no
...
- **R8.8 — Adaptive resizability first pass.** MainActivity now explicitly
  ...
  tabletop pane modes, expanded 840dp+ tablets now promote to ClearCut's
...
- **R5.7a/b/c — Plugin format family + OpenFX descriptor + compatibility
  matrix.** `PluginRegistry` (.clearcut-template / .ncfx / .ncstyle / .cube /
...
- **R5.5c — Privacy dashboard data model.** `PrivacyDashboard` is the single
  source of truth for every data category ClearCut collects: location,
...
- **C.12 — Keyframe bezier graph data model.** `KeyframeBezierGraph` ships the
  ...
  unit-segment presets for all 12 ClearCut easings, and `rescale(seg, start,
...
- **Multi-sequence export now carries ClearCut layer opacity into Media3.** `VideoEngine` builds per-input compositor layer metadata and applies it through `ClearCutVideoCompositorSettings`, so visible video/overlay tracks keep their track opacity in the real Media3 composition path.
...
- **The native payload remains intentionally gated.** ClearCut does not vendor the 50+ MB AAR into the base app until the packaging path is explicit; the runtime still falls back to the built-in Whisper ONNX engine.
...
- **Split from accessibility actions is resilient.** When the playhead is not inside the focused clip, ClearCut moves it to a safe midpoint before invoking the same split path used by the toolbar.
...
- **HDR profile probing now lives in `EncoderCapabilityProbe`.** ClearCut classifies HEVC, AV1, VP9, and AV1-based Dolby Vision Profile 10 encoder profiles from `MediaCodecList`, including advertised resolution / bitrate envelopes and encoder names.
...
- **`TemplateMarketplaceEngine`** (C.15) — self-hostable registry with documented JSON schema v1. User-configurable registry URL (defaults to `clearcut.dev/marketplace/index.json`). Complements the existing `.clearcut-template` export/import path from v3.8.
...
- PNG output auto-routes to `Pictures/ClearCut/` via the existing `exportUsesImageCollection` check in `saveToGallery()` — no new MediaStore code.
...
- **TemplateManager template-id sanitization** — Imported template ids were used directly as filename via `File(templateDir, "$id.json")`. A hostile `.clearcut-template` with id `../../etc/passwd` would land outside the template directory (path traversal). `normalizeImportedTemplate` now sanitizes ids to `[A-Za-z0-9_-]` and mints a fresh UUID when sanitization changes the value.
...
### Housekeeping
- `versionCode 98 → 99`, `versionName 3.37.0 → 3.38.0` (build.gradle.kts, ClearCutApp.VERSION, README badge, app_version string, CLAUDE.md, MEMORY.md).
...
- `ClearCutApp.VERSION`, `app_version` string, README badge all synced.
...
- `ClearCutApp.createNotificationChannels` API guard — `minSdk = 26` matches the API level where `NotificationChannel` was added; no guard needed.
...
### Housekeeping
- `versionCode 96 → 97`, `versionName 3.35.0 → 3.36.0` (build.gradle.kts, ClearCutApp.VERSION, README badge, app_version string, CLAUDE.md, MEMORY.md).
...
- **`@string/app_version` synced to `3.34.0`** — the resource was stuck on `v3.30.0` for several releases. It's not currently referenced from code (Settings already uses `ClearCutApp.VERSION`), but it's the kind of surface that appears in Play Store screenshots / accessibility scans when stale.
...
### Housekeeping
- `versionCode 94 → 95`, `versionName 3.33.0 → 3.34.0` (build.gradle.kts, ClearCutApp.VERSION, README badge, app_version string, CLAUDE.md, MEMORY.md).
...
- `ClearCutApp.VERSION` updated to match for HTTP user-agent strings on model downloads
...
- **Scoped-storage backup export** — Backup `.clearcut` files are now written via MediaStore on API 29+ instead of direct filesystem access, fixing permission failures on Android 11+.
...
### Export
- **Named output files** — Exports now use the project name (or batch item name) instead of `ClearCut_<timestamp>`, with automatic `(2)`, `(3)` collision avoidance.
...
- **GIF saves to Pictures** — GIF exports now save to Pictures/ClearCut instead of Movies/ClearCut
...
### Brand Refresh
- Replaced the launcher icon with a new ClearCut adaptive mark built around a luminous `N`, a precision cut stroke, and a nova spark.
- Added Android monochrome themed icon support and a matching round adaptive icon resource.
- Added reusable brand assets at `docs/branding/clearcut-icon.svg` and `docs/branding/clearcut-logo.svg`.
