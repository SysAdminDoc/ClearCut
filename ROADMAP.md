...
- [x] **CI on push/PR** — `build.yml` now fires on `push: branches: [master]`, `pull_request: branches: [master]`, plus the existing `workflow_dispatch` / `tags/v*`. Concurrency group cancels stale runs.
- [x] **`.gitignore` truth + tracked-files audit** — `ROADMAP.md` / `CHANGELOG.md` / `CODEX_CHANGELOG.md` removed from `.gitignore` (they were already tracked); HostShield + `research/` stay gitignored with a comment explaining the cross-project pollution risk. New `TrackedFilesAuditTest` runs `git ls-files` and fails the build if forbidden prefixes appear or if canonical planning docs go missing.

### Batch 2 — Data safety
- [x] **Autosave schema version field** — `serialize()` now writes both `version` and `schemaVersion` set to `FORMAT_VERSION`. New `AutoSaveState.peekSchemaVersion(raw)` is a pure top-level read that prefers `schemaVersion`, falls back to `version`, returns null on missing/malformed. New `ProjectAutoSave.LoadOutcome` sealed class (`Loaded` / `FutureSchema` / `Corrupt` / `NotFound`) + `loadRecoveryDataWithOutcome(projectId)` orchestrator refuses to fall through to the corrupt-recovery backup branch when either main or backup is from a newer schema. Existing `loadRecoveryData` keeps its `AutoSaveState?` contract for back-compat. 7 new tests in `AutoSaveSchemaVersionTest`.

### Batch 3 — Missing-media relink probe (engine layer)
- [x] **`MediaRelinkProbe` engine + 10 JVM tests** — New singleton with a `UriOpener` seam so the pure decision helper `check(clipId, uri, opener)` is JVM-testable without Android. Resolves to `RelinkState.OK / MISSING / UNKNOWN`. `probeClips(tracks)` recurses compound clips and returns `Map<clipId, ClipRelinkReport>` keyed by clip ID for cheap Timeline overlay lookup. Decision rules locked: positive length → OK; -1 (Photo Picker / SAF "unknown length") → OK; 0 → MISSING; null descriptor → MISSING; thrown SecurityException/IOException → MISSING with truncated reason; unsupported scheme (`ftp://`, `data:`, schemeless absolute path) → UNKNOWN. **Clip model unchanged**; UI integration follows in a later batch.

### Batch 4 — Pre-export AI provenance preview
- [x] **ExportSheet third confidence row "AI use"** — Engine: `AiUsageLedger.summarizeForChips(entries)` returns a stable-ordered `List<Chip>` (severity descending then effect-kind alphabetical) with bucketed entry count / clip count / total range / distinct model names. UI: new `AiUseConfidenceRow(chips, onChipClick)` composable in `ui/export/AiUseConfidenceRow.kt` renders the chip flow-row with severity-coloured badges (Peach for Disclose, Sky for Recommended, Green for Internal), per-chip body text from `Chip.describe()`, and an empty state. Drop into `ExportSheet.kt` confidence band with one call site. Strings: `export_ai_use_*`.

### Batch 5 — Photo Picker for sticker import
- [x] **Direct `PickVisualMedia(ImageOnly)` launcher for stickers** — New `stickerImageLauncher` in `EditorScreen` skips the full MediaPicker tab sheet for the common single-image overlay case. Per-URI grants survive background kill and no `READ_MEDIA_IMAGES` permission is requested. Plugin file imports (`.ncfx` / `.ncstyle` / `.ncfxd`) continue to use `ACTION_OPEN_DOCUMENT` as intended.

### Batch 6 — App Shortcuts + extended share intents
- [x] **App Shortcuts** — `res/xml/shortcuts.xml` ships two static entries (`new_project`, `recent_projects`) both routed to `MainActivity`; manifest registers them via `<meta-data android:name="android.app.shortcuts">`. New action constants `MainActivity.ACTION_NEW_PROJECT` / `ACTION_OPEN_RECENT` land on the Projects gallery. Dynamic "Resume recovered draft" shortcut pushed via `ShortcutManagerCompat.pushDynamicShortcut` from `ProjectListViewModel` is a follow-up (DataStore plumbing needed first).
- [x] **`AndroidManifest.xml` image/* and audio/* `ACTION_VIEW` filters** — Mirror the existing `video/*` filter, `content://` only, with `file://` rejection enforced by `handleViewIntent`. The mime-prefix allowlist (`video/`, `image/`, `audio/`) is a single named constant for easy extension. Per-type routing inside the Projects gallery (project import vs. add-to-current) is a small follow-up; pending-import field name `pendingVideoUri` kept for back-compat.

### Batch 7 — Diagnostic ZIP timeline-shape opt-in
- [x] **`DiagnosticExportEngine.TimelineShape` + opt-in bundle entry** — New `summarizeTimelineShape(tracks)` pure function returns counts-only summary (`trackCount`, `totalDurationMs`, `perTrackClipCount[trackType, clipCount]`, `perEffectTypeCount`, `perTransitionTypeCount`). `exportDiagnosticBundle(..., timelineShape: TimelineShape? = null, ...)` adds `timeline-shape.json` to the ZIP only when the caller passes a non-null shape. Hand-rolled JSON writer escapes `\`, `"`, control chars; 6 new tests in `DiagnosticTimelineShapeTest` lock the "no clip names / URIs / captions" invariant by injecting secrets into clip fields and asserting absence in JSON output. Existing Settings caller unaffected (positional arg).

### Batch 8 — Pre-flight model-availability sheet
- [x] **`AiToolRequirements` registry + `AiModelRequirementSheet` composable** — Engine: 16-tool requirement table with `requirementFor(toolId)` returning `ToolRequirement(modelDisplayName, estimatedBytes, license, sourceUrl, runtimeLocation, availability, requiresOptInConsent)`. UI: new `AiModelRequirementSheet(requirement, onDismiss, onDownload, onRun, onReviewModels)` in `ui/editor/AiModelRequirementSheet.kt` renders an `AlertDialog` with availability-aware title + icon + accent (Verified/Green for READY, Download/Sky for MODEL_DOWNLOAD_REQUIRED, CloudOff/Peach for DEPENDENCY_MISSING, Cloud/Yellow for CLOUD_OPT_IN), download size + license + runtime meta-rows, and a consent checkbox that gates the primary CTA when `requiresOptInConsent`. `AiToolsPanel` adoption is one wiring call site in `EditorScreen`. Strings: `ai_requirement_title_*` / `body_*` / `runtime_*` / actions.

### Batch 9 — Compound clip open/exit gesture (Tier C.13 UI)
- [~] **Compound nav UI helpers shipped** — Engine gains `canPush(clip)` (non-throwing predicate: false for non-compound, cycle, or over MAX_DEPTH) so the radial menu can grey itself out instead of catching exceptions, and `formatBreadcrumb(rootLabel, separator, fallbackParentName)` returning the single-line breadcrumb string ready for the chip composable. 7 new tests in `CompoundNavStackUiHelpersTest`. Timeline gesture + RadialActionMenu wiring + EditorViewModel `openCompoundClip(clipId)` orchestrator + predictive-back gate on `stack.depth > 0` follow in a focused UI commit consuming these helpers.

### Batch 10 — Caption translation editor row UI (Tier R5.4a UI)
- [~] **Editor-row state machine helpers shipped** — `CaptionTranslationEngine` gains `EditorRow(index, segment, quality)` view-model + four immutable list operations: `buildEditorRows()` (computes (variant, src, tgt) quality once and zips indices), `applyUserEdit()` (mutates only target text + flips state to USER_EDITED), `markRegeneratePending()` (preserves stale target while waiting), `completeRegenerate()` (replaces target, preserves word timings when new ones aren't provided). 7 new tests in `CaptionTranslationEditorRowsTest`. Compose two-up panel + language chip + per-row Regenerate UI follow in the panel commit.

### Batch 11 — Cut Assistant + Filler Removal merge
- [x] **Chip-bucket helpers + `CutAssistantFilterChips` composable** — Engine: `SilenceDetectionEngine.ProposalCategory { SILENCE / SINGLE_WORD_FILLER / MULTI_WORD_FILLER / OTHER }` + `categorize` / `filterByCategory` / `groupByCategory`. UI: new `CutAssistantFilterChips(proposals, enabled, onToggle, onToggleAll)` composable renders a `FlowRow` chip strip with per-bucket counts derived from `groupByCategory`; zero-count categories are hidden, the "All" chip is a convenience toggle. Caller-owned state via standard `mutableStateOf<Set<ProposalCategory>>`. Drop into `CutAssistantReviewPanel` filter band; the `FillerRemovalPanel` deletion is the remaining wiring step (route `AiToolsDelegate` filler-removal action through `CutAssistantReview`). Strings: `cut_filter_*`.
...
- [x] **`ProjectShortcutPlanner` + `ProjectListViewModel` wire-up** — Engine: pure planner with cases locked under 9 tests. UI side effect: `ProjectListViewModel.init { allProjects.collect { refreshDynamicShortcuts(it) } }` computes the last project + recovery state and calls `ShortcutManagerCompat.setDynamicShortcuts(appContext, list)`. Planner output maps 1:1 to `ShortcutInfoCompat` via the `toShortcutInfoCompat(ctx)` extension; launcher-rejection caught and logged so a misbehaving OEM launcher never crashes the app. Reuses `R.mipmap.ic_launcher` icon — no extra rasters needed.
...
Done in the autonomous continuation after R7.2: the AGP-8.7-compatible train now pins Compose BOM 2026.05.00, Dagger Hilt 2.58, AndroidX Hilt 1.3.0, Room 2.7.2, Coroutines 1.11.0, Lifecycle 2.10.0, DataStore 1.2.1, WorkManager 2.11.2, ONNX Runtime 1.26.0, MediaPipe Tasks Vision 0.10.35, OkHttp 5.3.2, and Lottie Compose 6.7.1. The train deliberately leaves AGP / Kotlin / KSP unchanged and holds Core KTX, Activity Compose, and Navigation Compose at the prior versions because latest stable AndroidX metadata requires AGP 8.9.1+. It also holds Dagger Hilt below 2.59.x because that line requires AGP 9.0+, and Room below 2.8.x because Room 2.8.4 trips the current Kotlin 2.1.0 / KSP 2.1.0 schema serializer path. The Hilt Compose import moved to `androidx.hilt.lifecycle.viewmodel.compose.hiltViewModel`, the Project Card swipe-to-delete flow no longer uses the deprecated `confirmValueChange` callback, and `scripts/check_16kb_alignment.py` now follows Google's `LOAD ... align 2**14` criterion instead of rejecting valid nonzero virtual addresses. Verification: `:app:testDebugUnitTest`, `:app:assembleDebug`, `scripts/check_16kb_alignment.py app/build/outputs/apk/debug/app-debug.apk`, and SDK Build Tools `zipalign -c -P 16 -v 4 app/build/outputs/apk/debug/app-debug.apk`.
...
Done in the 2026-05-17 autonomous continuation: NovaCut pins `com.moizhassan.ffmpeg:ffmpeg-kit-16kb:6.1.1` from Maven Central, [FFmpegEngine](app/src/main/java/com/novacut/editor/engine/FFmpegEngine.kt) now calls FFmpegKit's async execution API, and the debug APK passes both `scripts/check_16kb_alignment.py` and SDK Build Tools `zipalign -c -P 16`. License posture is explicit: the Maven POM declares LGPL-3.0, but the resolved AAR packages GPLv3 `res/raw/license.txt` and GPLv3 source-offer `res/raw/source.txt`; [LICENSE](LICENSE) and [docs/models.md](docs/models.md) therefore treat redistributed builds with this AAR as GPLv3-obligation builds unless a future release flavor swaps to a verified LGPL-only/no-FFmpeg variant.
...
Done in the autonomous continuation after A.2: NovaCut now pins `androidx.media3:media3-effect-lottie:1.10.1` from Google Maven and routes eligible export-time Lottie overlays through [Media3LottieTextureOverlay](app/src/main/java/com/novacut/editor/engine/Media3LottieTextureOverlay.kt). The adapter uses Media3's official `androidx.media3.effect.lottie.LottieOverlay` renderer while preserving NovaCut's existing timeline semantics: animation time is relative to the overlay start, alpha is zero outside the overlay window, `TextDelegate` substitutions still apply, and the Lottie canvas scales to the active video frame. The custom [LottieOverlayEffect](app/src/main/java/com/novacut/editor/engine/LottieOverlayEffect.kt) remains as a guarded fallback for HDR exports and over-long title windows because Media3 `OverlayEffect` treats non-text `BitmapOverlay` instances as Ultra HDR overlays under HDR input, and the official `LottieOverlay` loops animations while NovaCut's shader holds the final frame. The repo currently has no committed `app/src/main/assets/lottie_templates/` payloads, so this pass verifies API/behavior parity with source inspection and JVM backend-selection tests rather than pixel-golden template renders. Sources: https://developer.android.com/jetpack/androidx/releases/media3 · https://developer.android.com/reference/androidx/media3/effect/lottie/LottieOverlay · https://developer.android.com/reference/androidx/media3/effect/lottie/LottieOverlay.Builder · https://dl.google.com/dl/android/maven2/androidx/media3/media3-effect-lottie/maven-metadata.xml

R6.10b also now has a source-backed decision: [PreviewPanelMedia3ComposePolicy](app/src/main/java/com/novacut/editor/ui/editor/PreviewPanelMedia3ComposePolicy.kt) and [docs/preview-media3-compose.md](docs/preview-media3-compose.md) keep the custom [PreviewPanel](app/src/main/java/com/novacut/editor/ui/editor/PreviewPanel.kt) because the 1.10.1 Material3 `Player` and `ProgressSlider` are player-timeline widgets, while NovaCut needs edited-timeline seeking, gap recovery, still-image fallback, transform gestures, scopes, and custom chrome. Sources: https://developer.android.com/media/media3/ui/compose-material3 · https://developer.android.com/reference/kotlin/androidx/media3/ui/compose/material3/indicator/ProgressSlider.composable · https://dl.google.com/dl/android/maven2/androidx/media3/media3-ui-compose-material3/maven-metadata.xml

R6.10c is also closed as an audit and migration guard. [FrameExtractionPolicy](app/src/main/java/com/novacut/editor/engine/FrameExtractionPolicy.kt) keeps small cached SDR timeline/contact-sheet thumbnails on `MediaMetadataRetriever`, but routes future HDR review frames, effect-aware thumbnails, and custom decoder selection to `androidx.media3:media3-inspector-frame` with the new `androidx.media3.inspector.frame.FrameExtractor` import. [docs/frame-extraction-media3-inspector.md](docs/frame-extraction-media3-inspector.md) inventories the current `extractThumbnail`, thumbnail strip, freeze-frame, and AI analysis paths. Sources: https://developer.android.com/media/media3/inspector · https://developer.android.com/jetpack/androidx/releases/media3 · https://dl.google.com/dl/android/maven2/androidx/media3/media3-inspector-frame/maven-metadata.xml

R6.10d is closed as a tested non-adoption. [TimelineProgressSliderPolicy](app/src/main/java/com/novacut/editor/ui/editor/TimelineProgressSliderPolicy.kt) keeps the custom [Timeline](app/src/main/java/com/novacut/editor/ui/editor/Timeline.kt) ruler because it maps zoom, scroll window, markers, snap targets, clip hit areas, and scrub lifecycle callbacks over the edited project timeline. [MiniPlayerBar](app/src/main/java/com/novacut/editor/ui/editor/MiniPlayerBar.kt) keeps the standard Material3 `Slider` because it is externally driven by `playheadMs / totalDurationMs` and calls NovaCut's `onSeek(Long)` callback. Sources: https://developer.android.com/reference/kotlin/androidx/media3/ui/compose/material3/indicator/ProgressSlider.composable · https://developer.android.com/media/media3/ui/compose-material3
...
| [R7.3](#r73---dependency-stabilization-train) | Dependency stabilization train | Done for the AGP-8.7-compatible slice: Compose BOM, Hilt, AndroidX Hilt, Room, WorkManager, Lifecycle, DataStore, Coroutines, ONNX Runtime, MediaPipe, OkHttp, and Lottie are updated with explicit AGP/Kotlin/16 KB gates documented. |
| [R7.4](#r74---ffmpeg-16-kb-and-license-decision-gate) | FFmpeg 16 KB and license decision gate | Done for the default debug/app integration: R6.5 pins `com.moizhassan.ffmpeg:ffmpeg-kit-16kb:6.1.1`, debug APK verification shows 30 required 16 KB-aligned native libs and 0 misaligned, and [LICENSE](LICENSE) / [docs/models.md](docs/models.md) now treat the resolved AAR as GPLv3 obligation material because the packaged resources include GPLv3 license/source-offer text despite the POM's LGPL-3.0 metadata. F-Droid can still swap to an LGPL-only/no-FFmpeg flavor later if required. |
| [R7.5](#r75---media3-lottie-adoption-experiment) | Media3 Lottie effect spike | Done: `media3-effect-lottie:1.10.1` is wired through `Media3LottieTextureOverlay` for non-HDR finite-window title overlays; the custom shader remains only as the documented HDR / no-loop fallback. |
| [R6.5](#r65--ffmpeg-kit-16kb-supersedes-r52a-block) | Pin `com.moizhassan.ffmpeg:ffmpeg-kit-16kb:6.1.1` for A.9 | Done: version catalog + app dependency are wired, [FFmpegEngine](app/src/main/java/com/novacut/editor/engine/FFmpegEngine.kt) bridges the existing helper methods through FFmpegKit async execution, and APK 16 KB checks pass. Remaining downstream work: route reverse export, libass burn-in, two-pass loudnorm analysis, sidechain ducking, and mixed copy/re-encode composer through this now-active engine. |
| [A.2](#tier-a--activate-scaffolded-stubs) | DeepFilterNet 3 activation (model bump) | Done: NovaCut now pins `io.github.kaleyravideo:android-deepfilternet:0.0.8`, verifies the bundled native AAR and final debug APK for 16 KB page readiness, decodes source audio to 48 kHz mono PCM through the active FFmpeg bridge, processes DeepFilterNet frames on Android, and re-encodes cleaned audio to M4A with pass-through fallback when either native runtime is unavailable. |
...
| [R6.10](#r610--media3-110-modular-ui-adoption) | Adopt Media3 1.10 modular pieces where parity holds | Done. Lottie uses the official renderer where parity holds; Player Compose and ProgressSlider full replacements are rejected for 1.10.1 because NovaCut needs edited-timeline controls; inspector-frame is documented as the future HDR/effect/custom-decoder frame path while current SDR thumbnails stay on `MediaMetadataRetriever`. |
...
| [R8.3](#r83--android-1516-edge-to-edge-audit-mandatory-at-targetsdk--36) | Android 15/16 edge-to-edge audit | Done: MainActivity calls `enableEdgeToEdge()` and now uses `safeDrawingPadding()` on the NavHost root (replaces `systemBarsPadding`, so display cutouts are covered alongside system bars); `Window.isNavigationBarContrastEnforced = false` (API 29+ guarded) drops the 3-button-nav scrim. Static audit found no remaining root screen outside that safe drawing envelope. |
| [R8.4](#r84--predictive-back-for-full-screen-editor) | Predictive back for editor | Done by existing pattern: AndroidManifest already declares `enableOnBackInvokedCallback="true"`; the EditorScreen `BackHandler` is gated on dirty-state predicates so the system gets the back gesture (predictive back-to-home) whenever there is nothing to dismiss. Compose's `BackHandler` auto-bridges to `OnBackInvokedDispatcher` on Android 13+. |
| [R8.7](#r87--per-app-language-preferences-android-13) | Enable per-app `LocaleConfig` | Done: manifest declares `android:localeConfig="@xml/locales_config"`; baseline `en` locale entry shipped. Adding a `res/values-<bcp47>/` folder + appending one `<locale>` line lights up that locale in the system per-app picker. |
| [R8.8](#r88--android-16-adaptive-resizability-tablets--foldables) | Adaptive resizability audit for sw≥600dp | Done: manifest explicitly opts into resizable activity behavior, expanded config changes, and activity-embedding split readiness; [AdaptiveEditorLayoutPolicy](app/src/main/java/com/novacut/editor/ui/editor/AdaptiveEditorLayoutPolicy.kt) classifies compact/medium/expanded/tabletop/desktop pane modes; 840dp+ tablets now get the existing desktop sidebar/roomier timeline sizing; MediaPicker accepts external drag/drop media URIs; WindowManager `FoldingFeature` posture drives tabletop mode; 1000dp+ three-pane windows render ExportSheet as an embedded side pane instead of covering the editor. |
| [R8.9](#r89--ai-disclosure-ux-pairs-with-c2pa-and-generative-policy) | AI-generated content disclosure UX | In progress: [AiUsageLedger](app/src/main/java/com/novacut/editor/engine/AiUsageLedger.kt) + autosave persistence are live; ExportSheet now defaults "Disclose AI use" on for any non-empty ledger, writes a matching `.ai-use.json` sidecar, and exposes a clear/review flow; active AI edit surfaces now record usage for background removal/replacement, style transfer, upscale assist, Auto Edit, and TTS; ExportDelegate also writes an unsigned `.c2pa-manifest.json` sidecar from `C2paExportEngine`; DirectPublishEngine includes AI disclosure text for YouTube/TikTok-style targets; PrivacyDashboard has an AI ledger row. Remaining: signed C2PA MP4 embedding, MP4 `udta` metadata, and real API-upload checkbox handling once those platform upload paths ship. |
| [R8.15](#r815--local-network-protection-mode-android-16) | LNP scope classifier for live streaming | Pre-flight done: [OutputStreamingEngine.classifyNetworkScope](app/src/main/java/com/novacut/editor/engine/OutputStreamingEngine.kt) + `requiresLocalNetworkPermission()` route any RTMP/SRT/RIST/WebRTC/RTSP URL into PUBLIC_INTERNET / LOCAL_LAN / MULTICAST / LOOPBACK before the streaming library is wired. Lets the future R6.17 UI gate the Android-16 LNP consent sheet on real LAN destinations only, not on Twitch/YouTube/etc. 9 new tests cover RFC1918 ranges, multicast, link-local, IPv6 heuristics, mDNS, malformed URLs. Manifest permission request is deferred until the actual streaming dep lands. |
...
| [R8.2](#r82--c2pa-content-credentials-for-mp4-export) | C2PA Content Credentials on MP4 export | Engine scaffold done: [C2paExportEngine](app/src/main/java/com/novacut/editor/engine/C2paExportEngine.kt) builds a structured `C2paManifest` (training-mining opt-out, thumbnail claim, `c2pa.created` action, AI-actions assertion derived from `AiUsageLedger`) with `SigningMode` ANDROID_KEYSTORE / STRONGBOX / USER_PEM / WEB_SERVICE. AI ledger entries are mapped to canonical C2PA actions (`c2pa.created` / `c2pa.edited` / `c2pa.opinion`) with v2 `digitalSourceType` (`trainedAlgorithmicMedia` / `compositeWithTrainedAlgorithmicMedia` / `algorithmicMedia`). `ExportDelegate` writes an unsigned `.c2pa-manifest.json` sidecar for AI-disclosed exports so the manifest spec is preserved before the signer lands. `signAndEmbed` returns `Unavailable` until the c2pa-android AAR is wired; remaining is the AAR addition + 16 KB verification + license addendum. |
| [R8.5](#r85--thermal-aware-export-scheduling) | Thermal-aware export scheduling | Service bridge done: [ExportService](app/src/main/java/com/novacut/editor/engine/ExportService.kt) registers `addThermalStatusListener` on API 29+, polls `getThermalHeadroom(30)` once per second on API 30+, feeds [ThermalHeadroomPolicy](app/src/main/java/com/novacut/editor/engine/ThermalHeadroomPolicy.kt), surfaces debounced thermal progress/warning notifications, clears them on recovery/terminal states, and hard-cancels only on `THERMAL_STATUS_SHUTDOWN`. Remaining: true Media3 encoder throttling/proxy downgrade, resumable pause markers, overnight WorkManager export, and sustained-performance mode once VideoEngine exposes safe control hooks. |
| [R8.6](#r86--photo-picker--selected-photos-compliance) | Photo Picker / Selected Photos compliance | Done by design: [MediaPicker.kt](app/src/main/java/com/novacut/editor/ui/mediapicker/MediaPicker.kt) already uses `ActivityResultContracts.PickVisualMedia` / `PickMultipleVisualMedia` exclusively; AndroidManifest declares **no** broad `READ_MEDIA_*` permissions; Photo Picker grants per-URI access by default, so Selected Photos compatibility-mode loss does not apply. Google Play "Photo and Video Permissions" policy is satisfied automatically. |
| [R8.10](#r810--stylus-handwriting-in-caption-text-fields) | Stylus handwriting in caption editor | Verified done: Compose BOM 2026.05.00 pulls `foundation` ≥ 1.7.0 so stylus handwriting is default-on for every `TextField`. A `KeyboardType.Password` grep across the codebase returned zero hits — no field opts out. Optional polish (`handwritingDetector` on timeline clip body for S Pen → caption editor handoff) tracked as a future UI refinement. |
| [R8.13](#r813--auto-backup-for-projects--templates-android-backup-service) | Android Auto Backup for project metadata + templates | Done in prior work: AndroidManifest already declares both `android:fullBackupContent` and `android:dataExtractionRules`; rules cover database, autosave, freeze frames, voiceovers, TTS, noise-reduced, and stabilized output; partial-write artifacts excluded; proxy / thumbnails / models / cache excluded by default. |
...
| A.2 | **DeepFilterNet noise reduction** — ML path for aggressive mode, replaces spectral-gate heuristic | [engine/NoiseReductionEngine.kt](app/src/main/java/com/novacut/editor/engine/NoiseReductionEngine.kt) | `io.github.kaleyravideo:android-deepfilternet:0.0.8` (Maven Central) | ~7.6 MB bundled model / ~25.7 MB AAR | Pass-through fallback when FFmpeg or DeepFilterNet is unavailable; spectral-gate sample helper remains for future offline fallback |
...
- **KaleyraVideo/AndroidDeepFilterNet** — https://github.com/KaleyraVideo/AndroidDeepFilterNet — Maven artifact + JNI wrapper. Current wiring: `implementation("io.github.kaleyravideo:android-deepfilternet:0.0.8")`, `NativeDeepFilterNet(context)`, fixed-frame 48 kHz mono PCM buffers, and `processFrame(ByteBuffer)`.
...
- [~] **R5.4d — Locale-aware caption font fallback.** *(In progress — policy layer shipped: [CaptionFontFallbackPolicy](app/src/main/java/com/novacut/editor/engine/CaptionFontFallbackPolicy.kt) maps BCP-47 / ISO-639-1 language tags to the recommended Noto subset family (SC / TC / JP / KR / Arabic / Hebrew / Devanagari / Bengali / Tamil / Thai). Locale-region-aware (zh-Hant → TC, zh-Hans → SC). `totalBundleBytes()` + `rendersWithSystemFontsOnly()` surface per-language disclosure data for the Settings UI. 17 new tests cover every supported language, case insensitivity, and unknown-language fallback. Actual Noto bundle install + renderer integration follows in a UI commit; bundling Noto CJK alone is ~20 MB per writing system so distribution lives behind Play Asset Delivery (R5.6a).)*
...
- [x] **R5.7a — Promote `.novacut-template` to first-class plugin format.** *(Done — [PluginRegistry](app/src/main/java/com/novacut/editor/engine/PluginRegistry.kt) now classifies `.novacut-template`, `.ncfx`, `.ncstyle`, `.cube`, `.3dl`, and `.ncfxd` under one share-intent + import surface. Longest-extension-first detection so `.ncfxd` wins over `.ncfx` false-positive. 10 new tests.)*
- [x] **R5.7b — OpenFX-style effect descriptor (read-only).** *(Done — [OpenFxDescriptor](app/src/main/java/com/novacut/editor/engine/OpenFxDescriptor.kt) defines the `.ncfxd` JSON schema and parser. Carries `novaCutEffectId`, `openfxId`, per-parameter `(novaCutName, openfxName, range, scale, offset, type)` mapping with `toOpenFx`/`fromOpenFx` round-trip math. Permissive parser skips invalid parameter entries instead of failing the whole file. 10 new tests cover conversion math, round-trip, schema version gate, malformed-input rejection, inverted-range skipping.)*
- [x] **R5.7c — Glaxnimate / Rive / Lottie compatibility matrix.** *(Done — [docs/templates.md](docs/templates.md) covers the plugin format family, the round-trip compatibility matrix for Lottie / dotLottie / Rive / Glaxnimate, the kind-driven validation pipeline, license hygiene rules, and reproducibility hooks. Lottie 7.x state-machine path documented as the recommended dotLottie target; A.13 Rive parking decision recorded.)*
...
- [x] **R6.1a — Audit every bundled `.so` for 16 KB alignment.** *(Done — [scripts/check_16kb_alignment.py](scripts/check_16kb_alignment.py) is a pure-Python ELF parser that walks a directory tree, APK, or AAB and flags any arm64-v8a / x86_64 / riscv64 PT_LOAD segment whose `p_align` is < 0x4000 or whose `p_offset` / `p_vaddr` values are not congruent modulo `p_align`. CI workflow [.github/workflows/build.yml](.github/workflows/build.yml) now runs the script over the merged release native libs after `assembleRelease` and fails the build on misalignment.)*
...
The R5.2a R&D pass blocked on "no pinnable Maven artifact for ffmpeg-kit successor". As of 2026, `moizhassankh/ffmpeg-kit-android-16KB` publishes to Maven Central as `com.moizhassan.ffmpeg:ffmpeg-kit-16kb:6.1.1`, and the resolved debug APK proves the Android 15+/16 KB page-size requirement locally.

- [x] **R6.5a — Pin `com.moizhassan.ffmpeg:ffmpeg-kit-16kb:6.1.1`** in [gradle/libs.versions.toml](gradle/libs.versions.toml), add `implementation(libs.ffmpeg.kit.sixteenkb)`, and replace the `FFmpegEngine` stub method bodies with FFmpegKit async bridges. *(Done — [FFmpegEngine](app/src/main/java/com/novacut/editor/engine/FFmpegEngine.kt) now executes raw commands and structured argument lists, wraps cancellation through the FFmpegKit session, handles content-URI input through SAF parameters, and implements the existing stream-copy trim, concat demuxer, speed change, WAV extraction, subtitle burn-in, and loudnorm helper paths. JVM unit-test runtime still reports FFmpeg unavailable so native Android libraries are not called from plain JVM tests.)*
- [ ] **R6.5b — Route downstream features through the now-active engine:** B.3 reverse playback in export (`filter_complex` `[0:v]reverse[v]`), libass subtitle burn-in replacing the Canvas path on demand, exact two-pass `loudnorm` JSON analysis, sidechain compress ducking, AV1 software-encode fallback on devices without hardware AV1, and B.5 mixed copy/re-encode segment stitching.
- [x] **R6.5c — GPL note:** [LICENSE](LICENSE) now documents the FFmpegKit/FFmpeg notice and source-offer obligation. The resolved Maven POM declares LGPL-3.0, but the packaged AAR/APK includes GPLv3 `license.txt` and GPLv3 `source.txt`, so redistributed builds that include this AAR are treated as GPLv3-obligation builds unless a future flavor swaps to a verified LGPL-only/no-FFmpeg variant. Sources: https://github.com/moizhassankh/ffmpeg-kit-android-16KB · https://central.sonatype.com/artifact/com.moizhassan.ffmpeg/ffmpeg-kit-16kb · https://www.itpathsolutions.com/ffmpegkit-shutdown-what-to-do-next
...
- [x] **R6.6a — When A.2 activates, target DeepFilterNet 3 model weights** (not v2). *(Done — [NoiseReductionEngine](app/src/main/java/com/novacut/editor/engine/NoiseReductionEngine.kt) class docstring codifies the DeepFilterNet 3 target and companion constants expose the target model/version/source metadata for diagnostic surfaces.)* Sources: https://github.com/Rikorose/DeepFilterNet · https://github.com/KaleyraVideo/AndroidDeepFilterNet · https://github.com/KaleyraVideo/DeepFilterNet/blob/main/models/deepfilternet_model_optimization.md
- [x] **R6.6b — Activate the Android DeepFilterNet runtime.** *(Done — `gradle/libs.versions.toml` pins `io.github.kaleyravideo:android-deepfilternet:0.0.8`; the AAR metadata records SHA-256 `6566a208fe476a71b20558f92d93a1c0db49fd93b36fcdaea17a10260189d167` and tag `v0.0.8` maps to commit `42ea9b786babf7d67008a81cf25257b4735e4127`. AAR preflight found bundled `res/raw/deep_filter_mobile_model` (SHA-256 `5600b6857117ecc7cf460b8ec4841963bfa6d718921d424d42dea5d3d37a8c32`) and 16 KB-aligned arm64/x86_64 `libdf.so`; the final debug APK check reports 32 OK native libs, 40 skipped 32-bit libs, and 0 misaligned libs. [NoiseReductionEngine](app/src/main/java/com/novacut/editor/engine/NoiseReductionEngine.kt) now gates native availability off plain JVM, decodes input through [FFmpegEngine](app/src/main/java/com/novacut/editor/engine/FFmpegEngine.kt) to 48 kHz mono PCM, processes fixed-size direct `ByteBuffer` frames, re-encodes cleaned PCM to M4A, and falls back to pass-through if either native engine is absent or fails.)* Sources: https://central.sonatype.com/artifact/io.github.kaleyravideo/android-deepfilternet · https://repo.maven.apache.org/maven2/io/github/kaleyravideo/android-deepfilternet/maven-metadata.xml · https://github.com/KaleyraVideo/AndroidDeepFilterNet
...
- [x] **R6.10a — Swap custom `LottieOverlayEffect` for `media3-effect-lottie`** module where output parity holds. *(Done — `gradle/libs.versions.toml` now pins `androidx.media3:media3-effect-lottie:1.10.1`; [Media3LottieTextureOverlay](app/src/main/java/com/novacut/editor/engine/Media3LottieTextureOverlay.kt) adapts the official `androidx.media3.effect.lottie.LottieOverlay` renderer for non-HDR finite-window exports, keeps `TextDelegate` substitutions, scales the official canvas to the current video frame, and gates alpha with NovaCut's timeline window. [VideoEngine](app/src/main/java/com/novacut/editor/engine/VideoEngine.kt) keeps [LottieOverlayEffect](app/src/main/java/com/novacut/editor/engine/LottieOverlayEffect.kt) only when Media3 would change semantics: HDR exports, because Media3 `OverlayEffect` expects non-text `BitmapOverlay` HDR overlays to carry gainmaps, and title windows longer than the composition duration, because the official overlay loops while NovaCut's shader holds the final frame.)* Sources: https://developer.android.com/jetpack/androidx/releases/media3 · https://developer.android.com/reference/androidx/media3/effect/lottie/LottieOverlay · https://developer.android.com/reference/androidx/media3/effect/lottie/LottieOverlay.Builder · https://dl.google.com/dl/android/maven2/androidx/media3/media3-effect-lottie/maven-metadata.xml
- [x] **R6.10b — Evaluate `media3-ui-compose-material3` Player Composable** to replace bespoke `PreviewPanel` controls. *(Done — [PreviewPanelMedia3ComposePolicy](app/src/main/java/com/novacut/editor/ui/editor/PreviewPanelMedia3ComposePolicy.kt) codifies the non-adoption decision and [PreviewPanelMedia3ComposePolicyTest](app/src/test/java/com/novacut/editor/ui/editor/PreviewPanelMedia3ComposePolicyTest.kt) keeps it regression-tested. Keep NovaCut's custom `PreviewPanel`: Media3 1.10.1 offers a Material3 `Player`, playback controls, `ContentFrame`, `PlayerSurface`, and `ProgressSlider`, but `ProgressSlider` is player-position based and internally performs `Player.seekTo`; NovaCut's preview requires project-timeline seeking, gap recovery, still-image fallback, transform gesture callbacks, scopes, and Mocha chrome. Targeted future adoption can revisit lower-level `ContentFrame` / `PlayerSurface` or individual playback buttons, but not a full Player replacement yet.)* Sources: https://developer.android.com/media/media3/ui/compose · https://developer.android.com/media/media3/ui/compose-material3 · https://developer.android.com/reference/kotlin/androidx/media3/ui/compose/material3/indicator/ProgressSlider.composable · https://dl.google.com/dl/android/maven2/androidx/media3/media3-ui-compose-material3/maven-metadata.xml
- [x] **R6.10c — Track the `media3-inspector-frame` migration** (the old `FrameExtractor` moved out of `media3-inspector` in 1.10). Audit `extractThumbnail` paths. *(Done — [FrameExtractionPolicy](app/src/main/java/com/novacut/editor/engine/FrameExtractionPolicy.kt) records the split-module coordinate `androidx.media3:media3-inspector-frame`, the new import path `androidx.media3.inspector.frame.FrameExtractor`, and the deprecated import paths to avoid. [FrameExtractionPolicyTest](app/src/test/java/com/novacut/editor/engine/FrameExtractionPolicyTest.kt) pins the policy, confirms no old imports are used, and confirms no unused inspector-frame dependency was added. [VideoEngine](app/src/main/java/com/novacut/editor/engine/VideoEngine.kt)'s cached SDR timeline thumbnail and JPEG freeze-frame paths stay on `MediaMetadataRetriever`; HDR review frames, effect-aware thumbnails, or custom decoder selection should migrate to Media3 FrameExtractor.)* Sources: https://developer.android.com/media/media3/inspector · https://developer.android.com/jetpack/androidx/releases/media3 · https://dl.google.com/dl/android/maven2/androidx/media3/media3-inspector-frame/maven-metadata.xml
- [x] **R6.10d — `ProgressSlider` Composable** could replace the timeline ruler's progress indicator. *(Done — [TimelineProgressSliderPolicy](app/src/main/java/com/novacut/editor/ui/editor/TimelineProgressSliderPolicy.kt) codifies the non-adoption decision and [TimelineProgressSliderPolicyTest](app/src/test/java/com/novacut/editor/ui/editor/TimelineProgressSliderPolicyTest.kt) pins the contract. Keep the custom timeline ruler: it depends on edited project time, scroll offset, zoom, markers, snapping, clip hit targets, and scrub lifecycle callbacks. Keep `MiniPlayerBar`'s standard Material3 `Slider`: it is externally controlled by `playheadMs / totalDurationMs` and delegates to NovaCut's `onSeek(Long)`, while Media3 `ProgressSlider` is player-position based and performs player seek internally.)* Sources: https://developer.android.com/jetpack/androidx/releases/media3 · https://developer.android.com/reference/kotlin/androidx/media3/ui/compose/material3/indicator/ProgressSlider.composable · https://developer.android.com/media/media3/ui/compose-material3
...
- [x] **R6.11b — Surface a "Source is APV" chip** in ExportSheet (already chip-driven post-v3.74.3). *(Done — `ExportColorConfidenceEngine.summarizeSources()` now counts distinct inspected sources whose persisted `SourceColorMetadata.mimeType` is `video/apv`; the existing ExportSheet color-confidence chip row surfaces `Source is APV` with the large-file warning detail. 2 new tests cover the chip contract and source de-duplication.)*
...
- https://central.sonatype.com/artifact/io.github.kaleyravideo/android-deepfilternet — Maven Central artifact.
- https://repo.maven.apache.org/maven2/io/github/kaleyravideo/android-deepfilternet/maven-metadata.xml — Maven metadata for latest release.
- https://github.com/KaleyraVideo/DeepFilterNet/blob/main/models/deepfilternet_model_optimization.md — Kaleyra mobile DeepFilterNet3 optimization notes.
...
Done in the autonomous continuation after R8.7. Audit findings:
- [MainActivity.kt](app/src/main/java/com/novacut/editor/MainActivity.kt) already calls `enableEdgeToEdge()` in `onCreate`. ✓
- The NavHost root modifier now uses `safeDrawingPadding()` (was `systemBarsPadding()`) so every nav destination (projects, settings, editor) keeps its content out of system bars and display cutouts — no per-screen `Scaffold` rewrite required at this stage. ✓
- This pass adds `window.isNavigationBarContrastEnforced = false` (API 29+ guarded) so the 3-button-nav scrim no longer paints over edge-to-edge content. ✓
- Display cutout: static audit across the routed screen surfaces showed no remaining root screen outside the `safeDrawingPadding` envelope. Re-audit once R8.8 adaptive resizability work surfaces a Pixel Fold inner-display layout that intentionally uses the full horizontal width.
...
Done by existing pattern, verified during the R8 audit pass:
- AndroidManifest already declares `android:enableOnBackInvokedCallback="true"`, so the activity routes the back gesture through `OnBackInvokedDispatcher`.
- The single Compose `BackHandler` in tree — [EditorScreen.kt](app/src/main/java/com/novacut/editor/ui/editor/EditorScreen.kt) — is already gated on `hasOpenPanel || state.currentTool != EditorTool.NONE || hasClipSelection || isClipMode`. When the editor is idle, the handler is disabled so the system runs the back-to-home predictive animation users expect.
- The dismissal cascade inside that `BackHandler` (panels → tool → multi-select → single-select) is sequential and reversible; converting it to `PredictiveBackHandler { progress -> ... }` would only add a per-swipe animation rather than fix a correctness bug, so it's tracked as optional UX polish.
- Compose's `BackHandler` is auto-bridged to `OnBackInvokedDispatcher` on Android 13+, so no `onBackPressed` override regression is possible.
- Material3 ≥ 1.3.0 (R7.3 bumped to BOM 2026.05.00) already gives `ModalBottomSheet` and `SearchBar` first-party predictive-back animations.
...
Done in the 2026-05-17 autonomous continuation:
- `ExportService` registers `addThermalStatusListener` on Android 10+ export start and unregisters on cancel, completion, error, service destroy, or already-terminal state.
- `ExportService` polls `getThermalHeadroom(30)` once per second on Android 11+ and feeds the combined listener + forecast signal into `ThermalHeadroomPolicy`.
- The foreground progress notification now adds a compact thermal status text when the action is no longer `FULL_SPEED`; separate debounced warning notifications are raised for light/heavy/pause/stop transitions and cleared when the service returns to `FULL_SPEED`.
- NovaCut only hard-cancels the running export on `THERMAL_STATUS_SHUTDOWN`. `SEVERE` / headroom > 0.95 currently warn and keep the foreground export visible because Media3 `Transformer` does not expose a safe resumable pause/resume hook in NovaCut's current `VideoEngine`.

Remaining engine/UI targets:
- Add explicit `VideoEngine` thermal control hooks before claiming actual encoder throttling: reduce parallel filter work where NovaCut owns it, downgrade proxy tier before `Transformer.start`, and avoid pretending that sleeping the progress poller slows Media3 encoder threads.
- Add resumable pause markers for `SEVERE` / headroom > 0.95 only after export state can persist source composition, progress, and destination atomically.
...
Done by design — verified during the R8 audit pass and reframed below. The Round 8 entry assumed NovaCut requested the broad `READ_MEDIA_VIDEO` / `READ_MEDIA_IMAGES` / `READ_MEDIA_AUDIO` permissions (as the README's permission table implied). Live AndroidManifest does **not** request those broad permissions; [MediaPicker.kt](app/src/main/java/com/novacut/editor/ui/mediapicker/MediaPicker.kt) uses `ActivityResultContracts.PickVisualMedia` / `PickMultipleVisualMedia` and the system-provided `ACTION_OPEN_DOCUMENT` exclusively. Result:

- Google Play "Photo and Video Permissions" policy already passes — no broad permission to justify.
- Selected Photos compatibility-mode media loss does **not** apply — Photo Picker grants per-URI access that survives background kill through the URI permission grant model, no `READ_MEDIA_VISUAL_USER_SELECTED` needed.
- Re-grant CTA is not needed because no broad permission was ever requested.

Remaining (low-value): update the README's permission table to reflect that broad `READ_MEDIA_*` is **not** requested, so docs match the manifest. Tracked as a documentation cleanup, not engineering work.
...
Done in the autonomous continuation after R7.3. AndroidManifest now declares `android:localeConfig="@xml/locales_config"`, and the new [app/src/main/res/xml/locales_config.xml](app/src/main/res/xml/locales_config.xml) carries the `en` baseline locale. Adding a translation set is now a two-line change: a new `res/values-<bcp47>/strings.xml` file and one `<locale android:name="<bcp47>" />` entry in `locales_config.xml`. Verified via `:app:processDebugManifest` + `:app:processDebugResources`. Pairs with R5.4d Noto font fallback policy — `LocaleManager.applicationLocales` lets the caption font policy pick the correct script bundle without restarting the activity once translations land. Manual rather than AGP-auto was chosen so this works on AGP 8.7.3 without flipping experimental flags.
...
- Done in the autonomous continuation: audited `MainActivity` manifest posture. NovaCut now explicitly declares `android:resizeableActivity="true"` and handles `smallestScreenSize`, `density`, and `uiMode` config changes alongside orientation/screen-size changes. No orientation lock or restricted-resizability opt-out is declared; the editor embraces Android 16's forced-resizable behavior.
- Done in the autonomous continuation: [AdaptiveEditorLayoutPolicy](app/src/main/java/com/novacut/editor/ui/editor/AdaptiveEditorLayoutPolicy.kt) classifies `Compact` / `Medium` / `Expanded` width, compact/medium/expanded height, and `SINGLE_PANE` / `TWO_PANE` / `THREE_PANE` / `TABLETOP_SPLIT` pane modes. `LayoutMode` now promotes expanded 840dp+ tablet/foldable/desktop windows into the existing desktop sidebar mode even without a mouse signal, while preserving the one-handed phone preference under 600dp.
- Done in the autonomous continuation: `EditorScreen` uses the adaptive policy to give medium/expanded windows roomier preview and timeline heights instead of stretching the compact phone ratios.
- Done in the autonomous continuation: `MediaPickerSheet` accepts Compose drag-and-drop media URIs, imports them through the same local-copy path as the Photo Picker/OpenDocument flows, and keeps batch chronological sorting + failure messaging.
- Done in the autonomous continuation: `MainActivity` depends on AndroidX WindowManager, collects `WindowInfoTracker.windowLayoutInfo`, and provides a `LocalTabletopPosture` flag when a half-open horizontal `FoldingFeature` is present. `EditorScreen` feeds that posture into `AdaptiveEditorLayoutPolicy` and uses tabletop-specific preview/timeline sizing so foldable tabletop mode no longer relies on synthetic test inputs.
- Done in the autonomous continuation: the manifest declares `android.window.PROPERTY_ACTIVITY_EMBEDDING_SPLITS_ENABLED=true`, and `EditorScreen` now keeps the editor visible beside an embedded ExportSheet pane on 1000dp+ three-pane windows. AndroidX ActivityEmbedding itself splits activities into primary/secondary containers; NovaCut's export surface is a Compose panel sharing the live single-activity `EditorViewModel`, so introducing a second `ExportActivity` only to satisfy an embedding rule would risk duplicated export state. Revisit real `SplitPairRule` / `ActivityEmbeddingController` wiring only if export is intentionally extracted into its own Activity.
- Test with the Pixel Tablet and Pixel Fold series emulators in Android Studio with `targetSdkPreview = "Baklava"`.

- Sources: https://developer.android.com/develop/ui/compose/layouts/adaptive/app-orientation-aspect-ratio-resizability · https://developer.android.com/about/versions/16/behavior-changes-16 · https://developer.android.com/develop/ui/compose/layouts/adaptive/support-multi-window-mode · https://developer.android.com/guide/topics/ui/foldables · https://developer.android.com/codelabs/android-window-manager-dual-screen-foldables · https://developer.android.com/develop/ui/views/layout/activity-embedding · https://www.techtimes.com/articles/310767/20250612/android-16-brings-desktop-mode-tablets-foldables-get-left-behind.htm
...
Implementation target:
- Done in the autonomous continuation: new `AiUsageLedger` (in-memory + autosave-persisted) records every clip/range that touched disclosure-bearing or assistive AI. Ledger entries include clip ID, effect kind, model name, timeline range, and timestamp; helper methods classify severity, merge overlaps, produce a human-readable summary, and write a stable `com.novacut.ai-use.v1` JSON declaration. `EditorState`, `AutoSaveState`, `EditorViewModel.recordAiUsage`, and `clearAiUsageLedger` now carry the ledger without telemetry.
- Done in the autonomous continuation: Export sheet exposes a "Disclose AI use" toggle. It defaults **on** when the project has any non-zero ledger entry, shows the merged ledger summary, writes an optional matching `*.ai-use.json` sidecar, and lets the user clear the per-project ledger through a confirmation dialog.
- Done in the autonomous continuation: `DirectPublishEngine` carries AI disclosure text into share-intent fallback metadata and marks YouTube / TikTok as targets with AI-disclosure controls for the future API-upload path. `PrivacyDashboard` has a local, exportable, deletable AI ledger row.
- Done in the autonomous continuation: [AiUsageRecordFactory](app/src/main/java/com/novacut/editor/ui/editor/AiUsageRecordFactory.kt) maps successful AI edits to timeline-coordinate ledger entries. `AiToolsDelegate` records background removal/replacement, chroma-key background analysis fallback, style transfer, and upscale assist only after they mutate the project; `runAutoEdit` records the generated cut list as `AUTO_EDIT_LOCAL`; TTS adds `TTS_LOCAL` records for synthesized audio clips. Model-gated prompts do **not** write ledger entries until a tool actually changes media.
- Done in the autonomous continuation: `C2paExportEngine.manifestToJson` serializes the unsigned C2PA manifest spec deterministically, and `ExportDelegate` writes a sibling `*.c2pa-manifest.json` whenever AI-use sidecar export is enabled. This does not claim signed Content Credentials yet; it preserves the machine-readable manifest that the future c2pa-android bridge will sign and embed.
- Remaining: write disclosure metadata via two embedded media channels once the C2PA/export-metadata plumbing is ready:
  1. Signed C2PA manifest embedding (R8.2) — the manifest JSON is now generated; `signAndEmbed` still waits on the c2pa-android/simple-c2pa AAR and 16 KB native alignment/license review.
...
- Remaining: `DirectPublishEngine` API-upload mode should auto-tick the platform AI-content checkbox when the ledger is non-empty, with an explicit confirmation dialog before a user unticks it.
...
Verification done in the autonomous continuation after R8.7. A focused grep across `app/src/main/java` returned **zero** `KeyboardType.Password` declarations, so every shipping `TextField`/`BasicTextField`/`OutlinedTextField` (caption editor, project rename, chapter marker label, AutoEdit prompt, DesktopSidebar inputs) is eligible for the Compose foundation 1.7.0+ default-on stylus handwriting that ships with Compose BOM 2026.05.00. Stylus handwriting on captions, project names, marker labels, etc. should work out of the box on Android 14+ devices with a compatible IME (Gboard handwriting handles this on Pixel + Samsung S Pen devices).

Remaining as optional UX polish (no correctness gap): apply `handwritingDetector { ... }` to timeline clip body composables so S Pen → clip pre-focuses the caption editor at that timecode, mirroring the existing `StylusMidiEngine` jog/shuttle pattern. Same for the marker-list search field. Tracked as a future ~50 LOC refinement, no new dependency.
...
Done in prior work — verified during the R8 audit pass. The shipping AndroidManifest already declares both `android:fullBackupContent="@xml/backup_rules"` and `android:dataExtractionRules="@xml/data_extraction_rules"`. Existing [backup_rules.xml](app/src/main/res/xml/backup_rules.xml) and [data_extraction_rules.xml](app/src/main/res/xml/data_extraction_rules.xml) include `database`, `autosave`, `freeze_frames`, `voiceovers`, `tts_output`, `tts`, `noise_reduced`, and `stabilized`, and exclude every `*.partial.*` write-in-progress artifact. Proxy / thumbnails / model cache / diagnostics live outside those include paths so they're excluded by Auto Backup's default. The `data_extraction_rules` file scopes the same set for both `cloud-backup` and `device-transfer` blocks so D2D Migration carries the same payload as Google Drive backup.

Remaining (deferred, low-value): in-app "Disable Auto Backup" toggle in the Privacy Dashboard (R5.5c) and a 25 MB-cap autosave-history-trim suggestion are speculative additions; system Auto Backup already honors per-app user toggles in Settings. Re-open if a user-visible report surfaces backup-size pressure on a real project.
